 ...
 *   - `tag` (funnel axis) is preserved when the input was already an object that
 *     carried one — the two axes never clobber each other.
...
export function attachBrandFit(queries, brandFitByText) {
  ...
  return queries.map(item => {
    ...
    // Carry an existing funnel tag if the entry was already an object.
    const existingTag = (item && typeof item === 'object' && typeof item.tag === 'string' && item.tag.trim())
      ? item.tag.trim()
      : null;
    return existingTag ? { q: text, tag: existingTag, brandFit: fit } : { q: text, brandFit: fit };
  });
}
