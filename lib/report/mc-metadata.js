 ...
 *   - results[].degraded               (same class — WHICH grading model failed and why;
 *                                       the client-facing consequence is already said in
 *                                       words by lib/report/section-degradation.js)
...
import { computeComponents, computeUVI, smallSampleGuard, UVI_VERSION } from './visibility-index.js';
...
// AP-UVI-V2-SMALL-SAMPLE-GUARD (2026-09-20) — `uvi` is now the GUARDED
// composite: axes backed by fewer than SMALL_SAMPLE_MIN cells are dropped from
// the weighting and the rest re-normalised (see smallSampleGuard). The gate
// lives in visibility-index.js, NOT here, for the same reason the formula does:
// a second copy would let the MC payload and the report's UVI block publish two
// different numbers for one run. `uviVersion` ships alongside so a v1 baseline
// is never compared against a v2 re-measure as if they were the same metric,
// and `lowConfidence` names every axis that was measured but not counted.
function scores(summary) {
  ...
  return {
    ...
    lowConfidence: Array.from(guarded.values()).map(g => ({ axis: g.key, n: g.n, minSample: g.minSample })),
...
function perEngine(summary) {
  ...
  return Array.from(byProvider.values()).map(p => {
    const cells = p.cells;
    // ONE denominator (lib/score.js) — the Mission Control payload must publish
    // the same per-engine share the report does, computed the same way: hits
    // over VALID trials, never over every cell the run attempted.
    const { hits, total, rate: pct } = sliceStats(cells);
...
function perCell(summary) {
  ...
  return results.map(r => ({
    ...
    // AP-MENTION-ROLE-VENDOR-VS-SOURCE — `mention: 'yes'` alone cannot tell the
    // portal whether the answer offered the brand as a supplier or named it as
    // the author of a ranking about rivals. Absent on cells the role pass did
    // not classify, so an unclassified run's payload is unchanged.
    ...(r.mentionRole ? { mentionRole: {
      role: r.mentionRole.role,
      confidence: r.mentionRole.confidence || 'none',
    } } : {}),
...
function basket(summary, snapshots, config) {
  ...
  if (config && Array.isArray(config.basketHistory) && config.basketHistory.length > 0) {
    ...
    return {
      ...
      totalQueries,
      kind: basketKind(history),
      kindNote: freeTextKind(current),
    };
  }
...
/** @type {ReadonlySet<string>} The only values the consuming schema accepts. */
const BASKET_KINDS = new Set(['initial', 'additive', 'replace']);

/**
 * The basket change as a SCHEMA VALUE, never as prose.
 *
 * `basketHistory[].kind` is hand-written in `.aeo-tracker.json`, and in
 * practice it gets used as a human note ("locale-expansion: all 17 themes x
 * RU/PL/DE") rather than the enum the field is declared as. Passing that
 * through made the whole payload unloadable downstream — one free-text field
 * rejecting an otherwise valid run. The note is worth keeping, so it moves to
 * `kindNote` and the enum is derived from what actually happened to the query
 * set: the first entry is the initial basket; a later one that kept every
 * previous query added to it, otherwise it replaced part of it.
 *
 * @param {Array<{kind?: string, queries?: string[]}>} history
 * @returns {string} One of BASKET_KINDS.
 */
function basketKind(history) {
  const current = history[history.length - 1] || {};
  if (BASKET_KINDS.has(current.kind)) return current.kind;
  if (history.length <= 1) return 'initial';
  const previous = history[history.length - 2] || {};
  const before = Array.isArray(previous.queries) ? previous.queries : [];
  const after = new Set(Array.isArray(current.queries) ? current.queries : []);
  // No query lists to compare — "additive" is the honest default: the basket
  // demonstrably changed (there is a later entry) and nothing says anything was
  // dropped, which is the claim "replace" would make.
  if (before.length === 0 || after.size === 0) return 'additive';
  return before.every((q) => after.has(q)) ? 'additive' : 'replace';
}

/**
 * The author's own description of the basket change, when they wrote one
 * instead of an enum value. Null when `kind` was already a schema value.
 * @param {{kind?: string}} current
 * @returns {string|null}
 */
function freeTextKind(current) {
  const k = current && current.kind;
  return typeof k === 'string' && k && !BASKET_KINDS.has(k) ? k : null;
}
