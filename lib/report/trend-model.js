 ...
 *
 * WHAT THIS GATE DOES NOT CHECK — stated here because it has been misread.
 * This is a test on the coverage RATIO (`n / denominator`) on each side of the
 * pair. It is blind to WHICH questions those answers belong to: replace all 50
 * questions in a basket with 50 different ones and both ratios stay 1.0, so the
 * delta is allowed and reads as movement when nothing about the brand moved.
 * Only a change of SIZE, or of how much of the basket was measured, reaches it.
 *
 * Question identity is checked elsewhere, by the modules that own a pairing:
 * `lib/diff.js` and `lib/report/comparison-segments.js` key on the stable
 * question id from `lib/basket-manifest.js`, and `buildRunComparison` exposes
 * `basketChanged` / `counts.incomparableQuestions` from that work. Wiring those
 * into this gate is unfinished business (2026-09-15); until it is done, a
 * same-size basket swap passes this function.
 *
