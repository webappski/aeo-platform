...
import { formatModelTransition } from './model-change.js';
 ...
 * @param {Object} [modelChanges] Output of `buildModelChanges`. When an engine
 *   was swapped between the runs it becomes a driver alongside like-for-like and
 *   composition — never a reason to withhold the delta (see model-change.js).
 ...
 *            engineChange: Object|null}}
 */
export function summarizeDrivers(components, modelChanges = null) {
  ...
  return {
    ...
    gainPenalisedComponents: derived
      ...
      .map((comp) => comp.key),
    engineChange: engineChangeDriver(modelChanges),
  };
}

/**
 * The instrument swap as a DRIVER record — the same shape of answer the
 * conditional decomposition gives ("here is a share of the movement that is not
 * about the brand"), for the axis that had no decomposition at all.
 *
 * Returns null when no engine changed in a way that could explain movement, so
 * a renderer can test one field instead of restating the eligibility rule.
 *
 * @param {Object|null} modelChanges Output of `buildModelChanges`.
 * @returns {{providers: string[], transitions: Array<{provider: string, kind: string,
 *            strength: string, text: string, hasLikeForLike: boolean}>,
 *            noLikeForLike: string[], strongest: string, strongestKind: string}|null}
 */
export function engineChangeDriver(modelChanges) {
  const providers = modelChanges?.explanatoryProviders || [];
  if (providers.length === 0) return null;
  const relevant = (modelChanges.entries || []).filter((e) => providers.includes(e.provider));
  const RANK = { negligible: 1, moderate: 2, strong: 3 };
  // `strongestKind` travels WITH `strongest` because a renderer given only the
  // strength string cannot write true copy: `moderate` is shared by a genuine
  // small step (a minor version bump) and by "we could not tell how big this
  // was" (an unparseable pair). Describing the second with the first's wording
  // asserts a smallness nobody measured.
  const strongestEntry = relevant.reduce(
    (max, e) => ((RANK[e.strength] || 0) > (RANK[max?.strength] || 0) ? e : max),
    null,
  );
  return {
    providers,
    transitions: relevant.map((e) => ({
      provider: e.provider,
      kind: e.kind,
      strength: e.strength,
      text: formatModelTransition(e),
      hasLikeForLike: e.hasLikeForLike,
    })),
    noLikeForLike: modelChanges.noOverlapProviders || [],
    strongest: strongestEntry?.strength || 'negligible',
    strongestKind: strongestEntry?.kind || null,
  };
}
