...
// Same shared "could not read the answer" class + one-re-ask helper the
// competitor extractor uses — one policy, not two.
...
export function parseSentimentResponse(raw) {
  if (!raw || typeof raw !== 'string') {
    throw new LlmParseError('sentiment classifier returned empty response');
  }
  ...
  try {
    ...
    if (!m) throw new LlmParseError('sentiment response is not JSON and contains no {...} block');
    try { parsed = JSON.parse(m[0]); }
    catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      throw new LlmParseError(`sentiment response unparseable: ${msg}`);
    }
  }
  if (!parsed || typeof parsed !== 'object') {
    throw new LlmParseError('sentiment JSON malformed');
  }
  const label = String(parsed.label || '').toLowerCase().trim();
  if (!VALID_LABELS.has(label)) {
    throw new LlmParseError(`sentiment label "${parsed.label}" not in {positive, neutral, negative}`);
  }
...
export async function classifySentimentWithTwoModels({
  ...
  const runOne = async (p) => {
    const r = await runWithParseRetry({
      ...
      attempt: async (provider) => {
        const out = await classifyWithSingleModel({
          text, brand, domain,
        ...
        return { value: { label: out.label, rationale: out.rationale }, costInfo: out.costInfo };
      },
    });
    if (r.ok) {
      return {
        ok: true, label: r.value.label, rationale: r.value.rationale, costInfo: r.costInfo,
        ...(r.retriedOn ? { retriedOn: r.retriedOn } : {}),
      };
    }
    return {
      ok: false, label: null, rationale: null, costInfo: null,
      error: r.error,
  ...
  // What went wrong in each seat, in the typed form the degradation marker
  // needs. Attached ONLY when something actually went wrong or was re-asked, so
  // a clean cell keeps the exact shape every existing consumer reads.
  const seat = (p, res) => (p
    ? {
        provider: p.name,
        model: p.model,
        ...(res.error ? { error: res.error, errorKind: res.errorKind } : {}),
        ...(res.retriedOn ? { retriedOn: res.retriedOn } : {}),
      }
    : null);
  const noisy = (res) => !!(res && (res.error || res.retriedOn));
  const slots = (noisy(pRes) || noisy(sRes))
    ? { slots: { primary: seat(primary, pRes), secondary: seat(secondary, sRes) } }
    : {};
