...
export function renderHtml(summary, snapshots = null, opts = {}) {
  ...
  // allowed to state a delta (see `axisModel`).
  const AXIS_ORDER = ['presence', 'sentiment', 'rank', 'citation'];
  const axisHistories = Object.fromEntries(AXIS_ORDER.map(k => [k, []]));
  for (const snap of snaps) {
    let comps = null;
    try { comps = computeComponents(snap); } catch { comps = null; }
    for (const k of AXIS_ORDER) axisHistories[k].push(comps && Number.isFinite(comps[k]) ? comps[k] : null);
  }
  const axisModel = (() => {
    if (!latest) return { rows: [], metrics: [], shortCoverage: [] };
    let currBreak;
    try { currBreak = computeUVIBreakdown(computeComponents(latest)); }
    catch { return { rows: [], metrics: [], shortCoverage: [] }; }
    let prevBreak = null;
    if (prevSnapshot) {
      try { prevBreak = computeUVIBreakdown(computeComponents(prevSnapshot)); } catch { prevBreak = null; }
    }
    ...
    const rows = (currBreak.rows || []).map((row) => {
      ...
      if (cover.allowed) {
        metrics.push(buildMetric({
          ...
          history: axisHistories[row.key] || [], weight: row.weight, unitLabel: 'pp',
        }));
      }
      const chipHtml = !caps.chips
        ? ''
        : cover.allowed && delta != null
          ? L.chip(formatDelta(delta, 'pp'), chipTone(delta, 'points', true))
          : L.chip(cover.reason === 'coverage-shift' ? 'coverage moved' : 'too few to score', 'quiet');
      ...
      return {
        label,
        weight: row.weight,
        valueText: cover.allowed
          ? String(row.value)
          : `${row.sample?.n ?? 0} of ${row.sample?.denominator ?? 0}`,
  ...
  if (summary.positionMatrix && summary.positionMatrix.length > 0) {
    ...
    const rows = summary.positionMatrix.map((row, rowIndex) => {
      ...
      const cells = row.columns.map(col => {
        ...
        if (fullText && status !== 'missing' && status !== 'error') {
          ...
          const statusWord = status === 'yes' ? 'named'
            : status === 'src' ? 'cited'
              : (col.competitors || []).some(c => c && c.name) ? 'competitor named'
                : 'not named';
          const isExcerpt = !(typeof col.responseFull === 'string' && col.responseFull.trim());
          revealItems.push(
            `<details class="mx-reveal">`
            + `<summary><span class="mx-reveal-q">Q${rowIndex + 1}</span> `
            + `<span class="mx-reveal-eng">${esc(engLabel)}</span> `
            + `<span class="mx-reveal-status" data-status="${esc(status)}">${esc(statusWord)}</span>`
            + `${isExcerpt ? '<span class="mx-reveal-trunc">excerpt</span>' : ''}</summary>`
            + `<div class="mx-reveal-body"><p class="mx-reveal-meta">${esc(qLabel)} · ${esc(qText)}</p>`
            + `<pre class="mx-reveal-text">${esc(fullText)}</pre></div>`
            + `</details>`,
          );
        }
        if (status === 'yes') {
    ...
    visibilityCells.push(`
      <article class="cell span-6 dominant editor">
        ...
        <div class="matrix-grid" data-view="mention" data-sentiment-scored="${mxSentimentScored}" style="--cols: ${(summary.engines || []).length || 3}">
        ...
        </div>${revealItems.length > 0 ? `
        <div class="mx-reveals" aria-label="Full engine answers">
          <p class="mx-reveals-head">Read what each engine actually said — click any row to expand the full verbatim answer.</p>
          ${revealItems.join('\n          ')}
        </div>` : ''}
      </article>`);
  }
  ...
  const overviewLead = (() => {
    ...
    if (caps.shapes && (summary.trend || []).length >= 2) {
      const partialNote = partialFlags.some(Boolean)
        ? partialFlags.map((p, i) => (p ? `Run ${i + 1} measured ${snaps[i].results.length} answers of ${expectedCells}. Its score is marked on the chart and is not comparable.` : '')).filter(Boolean).join(' ')
        : '';
      const days = (() => {
      ...
      blocks.push(`<div class="lr-card">
        <div class="lr-chart-split">
          <div>
            ...
            ${partialNote ? `<p class="lr-mono-note">${esc(partialNote)}</p>` : ''}
          </div>
  ...
  // ── Visibility lead — answer by answer ────────────────────────────────
  const visibilityLead = (() => {
    if (!questionGroups.length) return '';
    const intro = `<p class="lr-sec-intro">${esc('Each row states its verdict in words. The panel behind it adds the two deltas and the whole record; the dots corroborate the sentence rather than replacing it. Rows ship open — that is the state a printed copy needs.')}</p>`;
    const cards = questionGroups
      .map(group => L.answerCard({ group, caps, runMeta: history.runs }))
      .join('');
    return intro + cards + actBar([presenceMetric], 'count metrics: 1-unit floor · no noise test');
  })();
  ...
  const citationsLead = (() => {
    ...
    const graphAlert = (() => {
      ...
      const broken = (entityGraph.edges || []).filter(e => e.status !== 'reciprocates');
      if (!broken.length) return '';
      const first = broken[0];
      const tiles = (entityGraph.edges || []).map(e => L.statusTile({
        name: e.host || e.platform || 'link',
        status: e.status === 'reciprocates' ? 'Reciprocates'
          : e.status === 'unreachable' ? 'Unreachable'
          : e.status === 'brokenLink' ? 'Broken link'
          : e.status === 'oneWay' ? 'One-way' : 'Not verified',
        tone: e.status === 'reciprocates' ? 'good' : e.status === 'unreachable' || e.status === 'brokenLink' ? 'bad' : 'warn',
      })).join('');
      return L.alertCard({
        tone: 'warn',
        ...
        bodyHtml: `Your schema declares ${entityGraph.sameAsCount} identity links. ${summaryEg.reciprocates} resolve and point back at you; ${broken.length === 1 ? 'the other returns' : 'the others return'} ${esc(broken.map(b => b.error || b.status).join(', '))} to a crawler, so your entity graph reciprocates at ${summaryEg.reciprocityRate}% instead of 100%. This is the cheapest authority signal on the board: point the link at a URL a crawler can actually read.`,
        extraHtml: `<div class="lr-alert-code">"sameAs": ["${esc(first.url)}"] → ${esc(first.error || first.status)}</div><div class="lr-tiles">${tiles}</div>`,
      });
    })();
  ...
  // ── Actions lead — internal build only ────────────────────────────────
  // §6 of the approved spec: the client deliverable ends at Diagnostics. The
  // gap matrix, per-engine playbooks and outreach drafts stay internal. The
  // white-label branch drops the section entirely (below), so this flag only
  // ever renders on the internal report — where it is a reminder, not a label
  // the client will read.
  const actionsLead = whiteLabel ? '' : `<span class="lr-internal-flag">Internal build only · not in the client deliverable</span>`;
  ...
  // header, the loud lead block, and the bento cells that follow it.
  //
  ...
  const SECTION_SPECS = [
    ...
    { id: 'visibility',  label: 'Visibility',  kicker: 'Visibility · per engine, by question',    title: visibilityTitle,
      ...
      lead: visibilityLead, cells: visibilityCells,
    ...
    { id: 'actions',     label: 'Actions',     kicker: 'Actions · what to ship next',             title: `${actionPlan.length} move${actionPlan.length === 1 ? '' : 's'}, in cost order`,
      meta: `gap matrix · run of ${summary.meta.date}`, tone: 'internal',
      lead: actionsLead, cells: actionsCells, internalOnly: true,
      emptyMsg: 'Action plan generates after the recommendations pass during report. Re-run report --html to populate.' },
  ];
  // Actions is internal-only per the standing founder decision; the client
  // deliverable ends at Diagnostics.
  const SECTIONS = SECTION_SPECS
    .filter(s => !(whiteLabel && s.internalOnly))
  ...
  const sectionsHtml = SECTIONS.map((s) => {
    ...
    const body = s.cells.length === 0
      ? (s.lead ? '' : (s.emptyMsg ? `<div class="bento">${sectionPlaceholder(s.emptyMsg)}</div>` : ''))
      : `<div class="bento">${s.cells.join('')}</div>`;
    if (!s.lead && !body) return '';
    return `<section id="${s.id}" class="lr-section">${head}${s.lead || ''}${body}</section>`;
  }).filter(Boolean).join('\n');

  // ── Rail nav (only sections that actually rendered) ──
  // A section now renders when it has EITHER a loud lead or bento cells —
  ...
  const railLinks = SECTIONS.filter(s => s.lead || s.cells.length > 0);
...
/* Print/PDF: expand EVERY verbatim-answer reveal so Save-as-PDF captures the
   full answers (the accordion is collapsed on screen). Toggle the native open
   attribute — the reliable cross-engine way to expand a details element; restore
   the on-screen collapsed state afterwards so the live page is unchanged. Falls
   back silently if matchMedia/print events are unavailable (older engines just
   print whatever is currently open). */
(function () {
  var reveals = Array.prototype.slice.call(document.querySelectorAll('.mx-reveal'));
