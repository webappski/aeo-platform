 ...
 */
function narrativeFor({ coverage, cited, topComp, citationsLeader, whiteLabel = false }) {
  const total = coverage.total || 0;
  const named = coverage.yes || 0;
  const srcCells = cited || 0;
  const ratio = total > 0 ? named / total : 0;
  // White-label client snapshots state the measured fact only — no «pitch first
  // to convert / push for first-position / start with crawlability / closing the
  // gap» recommendation clauses (advisory verbs break the self-made-tool legend).
  // The hero number itself stays identical; only the advisory tail is dropped.
  if (whiteLabel) {
    const citeTail = srcCells > 0
      ? ` Its domain was cited in <b>${srcCells} of ${total}</b> answers without the brand being named.`
      : '';
    if (named === 0 && srcCells === 0) {
      return `AI engines did not name <b>or</b> cite the brand in any of the <b>${total}</b> answers this run.`;
    }
    if (named === 0 && srcCells > 0) {
      return `Cited in <b>${srcCells} of ${total}</b> answers without being named — the domain appears in the source pool but is not yet promoted to a named brand.`;
    }
    if (ratio < 0.30 && topComp) {
      return `Named in <b>${named} of ${total}</b> answers; <b>${esc(topComp.name)}</b> was named ${topComp.count}× more across the same queries.${citeTail}`;
    }
    return `Named in <b>${named} of ${total}</b> answers.${citeTail}`;
  }
...
export function renderHtml(summary, snapshots = null, opts = {}) {
  ...
  const actionPlan = whiteLabel ? [] : assignDays(summary.actions || []);
  ...
  // The «Lift opportunities» KPI is advisory by label and its context copy
  // «pitch … first to convert / Pitch those publishers» is recommendation text.
  // White-label drops the whole cell (statistics-only); the mention-rate KPI
  // above carries the comparable number. Dropping the cell — not neutralising it
  // — removes any keyword-free advisory fragment risk entirely.
  if (!whiteLabel) {
    heroKpiCells.push(`
    <div class="hero-kpi">
  ...
  if (summary.trend.length < TREND_MIN_RUNS) {
    ...
    // The delta here is first-vs-last across the whole trend window — NOT
    // scoreDelta (vs previous run, hero panel). This panel's copy says
    // «across N runs», so a last-step dip must not be presented as the
    // multi-run trend (100→83 once printed as «Down 17 points across 6 runs»
    // while the window was up 50).
    const trendNums = summary.trend.filter(v => typeof v === 'number');
    const trendMax = Math.max(0, ...trendNums);
    const allZero = trendMax === 0;
    const runCount = summary.trend.length;
    const windowDelta = trendNums.length >= 2 ? trendNums[trendNums.length - 1] - trendNums[0] : null;
    const trendTitle = windowDelta > 0 ? 'Score is climbing'
      : windowDelta < 0 ? 'Score slipping'
      : allZero ? 'Flat at zero'
      : 'Score is steady';
    const lift = windowDelta != null
      ? windowDelta > 0 ? `Up ${windowDelta} points across ${runCount} runs.`
        : windowDelta < 0 ? `Down ${Math.abs(windowDelta)} points across ${runCount} runs.`
  ...
  const SECTIONS = whiteLabel ? [
    ...
    { id: 'competitors', num: '03', label: 'Competitors', subtitle: 'who AI named instead',                 cells: competitorsCells,
      emptyMsg: 'No competitors detected — the AI engines did not name brands in their answers this run.' },
    ...
    { id: 'diagnostics', num: '05', label: 'Diagnostics', subtitle: 'site readiness, ads',                  cells: diagnosticsCells,
