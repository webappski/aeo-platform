...
import {
  buildInstrumentCaveat, buildCoverageCaveat, coverageCaveatTitle, buildBasketCaveat,
  INSTRUMENT_ROW_NOTE,
...
import { aggregateScore, sliceStats } from '../score.js';
...
export function renderHtml(summary, snapshots = null, opts = {}) {
  ...
  // Markdown blocks embedded as HTML panels. ONLY what is read back below
  // belongs here: `sentiment`, `utm` and `ads` sat in this object until
  // 2026-09-21 and were never read (there was no `S.sentiment` / `S.utm` /
  // `S.ads` anywhere in this file), so three sections were built on every
  // render and thrown away. Each is superseded by a live HTML view that is
  // built inline and shows the same thing — the matrix sentiment view, the UTM
  // KPI, and the ad-signal stanza — which is why removing them changes not one
  // byte of the rendered page (proved on a seed with every block's own render
  // condition satisfied, all three modes). `funnel` is read again as of
  // 2026-09-21 and renders on the page when the run carries tagged queries.
  // It stays empty for every run recorded so far, because nothing authors a
  // tag yet — see TECH_DEBT item 2.
  const sectionsRaw = snapshots ? {
    funnel:     sectionFunnelBreakdown(snapshots, { whiteLabel }),
  ...
  // The two "was this even the same measurement?" caveats. Both are built from
  // `comparison` and rendered at the top of Overview. Until 2026-09-02 this
  // file computed `comparison` and read it nowhere: the model was correct, the
  // markdown report printed it, and the HTML report — the one a client is
  // shown — printed a movement figure with nothing beside it.
  const comparison = runCount > 1 ? buildRunComparison(snaps) : null;
  const instrumentCaveat = comparison ? buildInstrumentCaveat(comparison) : null;
  const coverageCaveat = comparison ? buildCoverageCaveat(comparison) : null;
  const basketCaveat = comparison ? buildBasketCaveat(comparison) : null;
  // The fourth axis of "was this even the same measurement?", and the only one
  // that needs no second run: what THIS run could not classify. Built from the
  // latest snapshot's own cells, so it fires on run 1 where the three above
  // cannot.
  const degradationCaveat = buildSectionDegradationCaveat(latest?.results, { whiteLabel });
  // Providers whose model moved in a way that can explain movement — used to
  // mark the individual blocks that assert who took the brand's place, not just
  // the section-level note.
  const swappedProviders = new Set(instrumentCaveat?.providers || []);
  ...
  if (summary.positionMatrix && summary.positionMatrix.length > 0) {
    ...
    // MEAS-2 — the fraction this block PUBLISHES ("N / M cells with your
    // brand") comes from the run's one denominator (lib/score.js), not from a
    // third count of matrix cells: `mxAgg.totalCells` includes the cells whose
    // engine call errored, so the same run used to show one share here and a
    // different one in the headline. The bucket counts above stay cell-level —
    // they describe the grid, they are not a rate. Falls back to the cell count
    // when a snapshot carries a matrix but no results (never-fail).
    const mxScore = aggregateScore(summary.results || []);
    const mxShareHits = mxScore.valid > 0 ? mxScore.hits : mxYours;
    const mxShareTotal = mxScore.valid > 0 ? mxScore.valid : mxAgg.totalCells;
    ...
    const rows = summary.positionMatrix.map((row, rowIndex) => {
      ...
      // Per-question row total through the ONE denominator (lib/score.js): an
      // engine that failed its call did not decline to name you, so it must not
      // sit in the row's denominator as if it had.
      const rowStats = sliceStats(row.columns);
      const rowNamed = rowStats.hits;
      const rowTotal = rowStats.valid;
    ...
    const summaryStats = [
      `<span class="mx-sum-stat" data-tone="${mxShareHits > 0 ? 'good' : 'muted'}"><strong class="mx-sum-num">${mxShareHits}</strong><span class="mx-sum-denom">/ ${mxShareTotal}</span><span class="mx-sum-label">${mxShareHits === 1 ? 'cell' : 'cells'} with your brand</span></span>`,
  ...
  // Funnel stage / intent tag — only when the run carries tagged queries.
  // `sectionFunnelBreakdown` returns '' when no cell has a `tag`, so this is a
  // no-op for every run recorded to date: no existing report gains a section
  // until someone tags queries. See TECH_DEBT item 2 — nothing authors a tag
  // yet, which is the other half of making this section reachable.
  if (S.funnel) {
    visibilityCells.push(`
      <article class="cell span-6 quiet">
        <div class="cell-head"><span class="cell-label">By funnel stage</span></div>
        ${S.funnel}
      </article>`);
  }
  ...
  const overviewLead = (() => {
    ...
    // (0) Was this the same instrument as last run, and were the same engines
    //     on it? Both caveats LEAD the section — deliberately, and deliberately
    //     above (a). On 2026-09-01 this report printed "42 points below the
    //     2026-08-13 run" and named the companies that had supposedly taken the
    //     brand's place, while ChatGPT had answered the two runs on two
    //     different models; the block that names competitors is (a), directly
    //     below. A note underneath it does not reach a reader who has already
    //     read the names — the same reason the markdown table carries its own
    //     per-row mark rather than a footnote (sections.js, rcLostBlock).
    //
    //     Ungated by the degradation ladder on purpose: this is a fact about
    //     the two runs in hand, not a trend claim, and it matters most on run 2
    //     where the ladder is at its most conservative.
    if (instrumentCaveat) {
      blocks.push(L.alertCard({
        tone: 'warn',
        kicker: 'A note on the measurement',
        title: instrumentCaveat.transitions,
        bodyHtml: esc([
          instrumentCaveat.explanation,
          instrumentCaveat.noLikeForLike,
          instrumentCaveat.closing,
        ].filter(Boolean).join(' ')),
      }));
    }
    if (coverageCaveat) {
      blocks.push(L.alertCard({
        tone: 'warn',
        kicker: 'A note on the coverage',
        title: coverageCaveatTitle(coverageCaveat),
        bodyHtml: esc(coverageCaveat.sentences.join(' ')),
      }));
    }
    // The basket axis, beside the instrument and coverage ones. Without it the
    // markdown report says the two runs asked different questions and this page
    // — built from the SAME model — says nothing, which is the "a caveat a
    // reader can shop around" failure this module's header describes.
    if (basketCaveat) {
      blocks.push(L.alertCard({
        tone: 'warn',
        kicker: 'A note on the questions',
        title: basketCaveat.title,
        bodyHtml: esc(basketCaveat.sentences.join(' ')),
      }));
    }
    // What this run could not classify. Beside the other three notes and built
    // from the same doctrine — one builder, identical words on both surfaces
    // (lib/report/section-degradation.js). Unlike them it needs no previous
    // run, which is exactly when a thinned competitor list is most likely to be
    // read as a finding.
    if (degradationCaveat) {
      blocks.push(L.alertCard({
        tone: 'warn',
        kicker: 'A note on what could not be classified',
        title: degradationCaveat.title,
        bodyHtml: esc(degradationCaveat.sentences.join(' ')),
      }));
    }
    ...
    if (changedCell && caps.chips) {
      ...
      blocks.push(L.alertCard({
        ...
        // Two independent reasons this one answer may not mean what it looks
        // like — the question was reworded, or the engine was not the same
        // engine. Both are stated here rather than only in the notes above,
        // because this card is where the report is at its most assertive: it
        // names a company as having taken specific ground.
        footHtml: [
          changedCell.textDrift
            ...
            : '',
          swappedProviders.has(changedCell.provider)
            ? esc(`${engine} ran a ${INSTRUMENT_ROW_NOTE} — part of what looks like a change of hands here is the engine itself answering differently. See the note on the measurement above.`)
            : '',
        ].filter(Boolean).join(' '),
      }));
    }
    ...
    if (axisModel.rows.length) {
      ...
      // Two numbers a reader can see at once must never differ without an
      // explanation — and the explanation must be the real one. This note used
      // to say the gap meant "the scoring rules changed since this run was
      // measured". That cause was never observed and cannot be: the headline
      // is `summary.score`, which every writer fills from
      // `aggregateScore().hits / valid` (bin:3989, sections.js:3538,
      // comparison-payload.js:118), while the composite is a weighted blend of
      // four axes. They are different measurements of the same run, not two
      // versions of one measurement, so they would differ even if no rule had
      // ever changed.
      const recomputed = (() => {
      ...
      const driftNote = (recomputed != null && indexMetric.current != null && recomputed !== indexMetric.current)
        ? `<p class="lr-note">${esc(`These are two different measurements of the same run, not one number scored twice. The headline ${indexMetric.current}% is the share of answers that named or cited you — a plain count, and what the chart and every comparison above use. The Unified Visibility Index of ${recomputed} blends four axes at fixed weights, so an axis measured on too few answers changes it without changing the share.`)}</p>`
        : '';
      blocks.push(`<div class="lr-card">
  ...
  // MEAS-2 — every sentence built from this number says "answers", so the
  // number has to be answers we actually received: valid trials from the run's
  // ONE denominator (lib/score.js). `history.cells.length` counts the cells the
  // run ATTEMPTED, so a run with a failed call published "1 of 3 answers name
  // you" while the headline score divided by 2. Falls back to the cell count
  // when a snapshot carries history but no results (never-fail).
  const answerCounts = aggregateScore(latest?.results || []);
  const totalCells = answerCounts.valid > 0 ? answerCounts.valid : history.cells.length;
  const compCount = (latest?.topCompetitors || []).length;
  const overviewTitle = indexMetric.deltaPrev == null
    ? `${indexMetric.current}% of answers named or cited you on the first run`
    : indexMetric.deltaPrev < 0
      ...
      : indexMetric.deltaPrev > 0
        ...
        : `Holding at ${indexMetric.current}%`;
  ...
  // of its own. Its MODEL (buildRunComparison) is what Overview reads.
  //
  // That last sentence was false between the redesign and 2026-09-02: the model
  // was built into a local and never read, so every caveat it carries — the
  // instrument swap, the coverage change, the weight basis — reached the
  // markdown report and no HTML reader. The two caveats that decide whether the
  // movement figures mean anything now render at the top of Overview, from the
  // shared sentences in `instrument-caveat.js`, so the surfaces cannot word the
  // same fact two ways. The rest of the model (the lost/gained tables, the
  // never-held list) stays markdown-only by design — the loud register tells
  // those through the answer record and the Competitors section instead.
  //
  // White-label drops the Actions section entirely (statistics-only deliverable)
  // and uses neutral empty-state copy — the default empty messages name the tool
  // ("Run aeo-platform run…", "Re-run report --html…"), which would leak through
  ...
  const verdictHeroHtml = (() => {
    ...
    const lede = (() => {
      const parts = [];
      parts.push(`${esc(summary.meta.brand)} was named or cited in <b class="lr-strong">${indexMetric.current ?? summary.score}%</b> of answers on ${esc(summary.meta.date)}`);
    ...
    const kpis = [
      L.kpiCard({
        label: 'Share of answers naming or citing you',
    ...
    if (!whiteLabel) {
      ...
      kpis.push(L.kpiCard({
        ...
        value: lift.cited,
        // The denominator the sentence below uses (valid trials — lib/score.js),
        // never the attempt count: a KPI whose fraction disagrees with its own
        // note is the drift this module exists to prevent.
        denom: `/ ${Number.isFinite(lift.valid) ? lift.valid : lift.total}`,
