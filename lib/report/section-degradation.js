...
export function buildSectionDegradationCaveat(results) {
  ...
  for (const e of recovered) {
    ...
    const where = e.retriedOn.some((x) => x !== 'same-model')
      ...
      : 'a second ask';
    sentences.push(
      `${SECTION_LABELS[e.section]}: ${n} came back unreadable the first time and ${e.cells === 1 ? 'was' : 'were'} recovered by ${where} — nothing was lost, listed so the extra call is visible.`,
    );
  }
