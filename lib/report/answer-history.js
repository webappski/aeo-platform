 ...
 *
 * Slot rather than question text — and DELIBERATELY different from
 * `comparison-segments.js`'s `cellKey` and `lib/diff.js`, which both key on the
 * question's stable id since 2026-09-15. (This comment used to claim the three
 * matched; that stopped being true when the other two were fixed, so it is
 * stated as the divergence it is.)
 *
 * The two answer different questions. A COMPARISON must never subtract two
 * different questions, so it keys on the question. A HISTORY is the record of a
 * tracked slot over time; keying it on the text would restart every record the
 * day a question is reworded, which is exactly when continuity matters most.
 * The rewording is not swallowed — see `textDrift` on the returned entry, which
 * is how a reader learns the slot's question changed under it.
 *
