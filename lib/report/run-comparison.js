...
export function buildRunComparison(snapshots) {
  ...
  const segments = annotateSegments(rawSegments);
  ...
  return {
    ...
    components,
    driverSummary: summarizeDrivers(components),
...
/**
 * Add `isNoise` (lost + gained) and `competitorShift` (lost only) to segment
 * entries without mutating `segmentCells`'s output.
...
function annotateSegments(segments) {
  return {
    ...
    gained: segments.gained.map((entry) => ({
      ...entry,
      isNoise: isNoiseChange(entry.before, entry.after),
    })),
  };
