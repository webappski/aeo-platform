...
export function buildRunComparison(snapshots) {
  ...
  return {
    ...
    weightBasis,
    // True when the two runs did not ask the same questions. Every figure in
    // this model that spans the pair — the UVI delta above included — is then
    // partly a change of basket rather than of visibility, and a renderer must
    // say so instead of presenting it as movement. Derived from the segments,
    // so it cannot disagree with the table underneath it.
    basketChanged: (rawSegments.incomparable || []).length > 0,
    modelChanges,
    // The coverage axis, sized. `modelChanges` names WHICH engines appeared on
    // only one side of the pair; this attaches how many answers each of them
    // takes out of (or adds outside) every lost/held/gained figure, which is
    // the part a reader needs to judge whether the comparison still means
    // anything. Null when both runs measured the same set of engines.
    coverageChange: buildCoverageChange(modelChanges, segments),
    segments,
    ...
    counts: {
      ...
      // Cells whose QUESTION only one of the two runs asked. Not a gain, not a
      // loss, not a "never" — no second measurement exists. Counted here so a
      // renderer can state the size of the basket change instead of leaving the
      // reader to assume the two runs asked the same thing.
      incomparable: (segments.incomparable || []).length,
      incomparableQuestions: new Set(
        (segments.incomparable || []).map((e) => e.queryId),
      ).size,
...
/**
 * Size the coverage change: for each engine present on only one side of the
 * pair, how many of this comparison's cells it accounts for.
 *
 * The count is read off `segments.indeterminate` rather than off the raw
 * snapshots because that is precisely the bucket a one-sided cell lands in
 * (`classifyCell` routes a null on either side there). Counting the snapshot's
 * own rows instead would agree today and drift the moment a cell errors for an
 * unrelated reason.
 *
 * DELIBERATELY NOT the `incomparable` bucket, even though those cells are also
 * left out of the tally. This figure answers "how many answers does THIS
 * ENGINE's absence account for", and a cell excluded because nobody asked that
 * question in the other run is not explained by the engine — attributing it
 * here would inflate the engine's share by the size of the basket change
 * (on the real 2026-08-27 → 2026-08-31 pair, by ~44 answers per engine). The
 * basket's own share is reported separately, by `buildBasketCaveat`.
 *
 * @param {{droppedProviders?: string[], addedProviders?: string[]}} modelChanges
 * @param {{indeterminate?: Array<{provider: string, label: string}>}} segments
 * @returns {{dropped: Array<{provider: string, label: string, cells: number}>,
 *            added: Array<{provider: string, label: string, cells: number}>}|null}
 */
function buildCoverageChange(modelChanges, segments) {
  const dropped = modelChanges?.droppedProviders || [];
  const added = modelChanges?.addedProviders || [];
  if (dropped.length === 0 && added.length === 0) return null;
  const rows = segments?.indeterminate || [];
  const size = (provider) => {
    const own = rows.filter((e) => e.provider === provider);
    return {
      provider,
      // The engine's own label as the run recorded it, falling back to the
      // shared lookup — so a report never prints a raw provider id it could
      // have named properly.
      label: own[0]?.label || providerLabel(provider),
      cells: own.length,
    };
  };
  return { dropped: dropped.map(size), added: added.map(size) };
}
...
/**
 * Detect whether the set of axes that actually CARRY WEIGHT differs between the
 * two runs — i.e. whether re-normalisation put the UVI on a different weight
 * basis in one of them.
 *
 * Two ways an axis drops out of the weighting, and both must count here:
 *   - it was never measured that run   → `computeUVIBreakdown().excluded`
 *   - UVI v2 small-sample guard        → measured, but too few cells to weight
 *
 * Reading only `excluded` (the v1 test) missed the second one: a run whose
 * sentiment rested on 2 cells re-normalises over 0.55 while its baseline
 * re-normalised over 1.00, and every axis's `appliedWeight` differs across that
 * shift — so subtracting the two contributions produces a number attributable
 * to the re-normalisation, not to movement. That is the AP-WEIGHT-BASIS defect
 * one axis over, and the fix is the same: withhold the delta, carry the caveat.
 *
...
function weightBasisNote(prevBreakdown, currBreakdown) {
  const weighted = (breakdown) => new Set(
    (breakdown.rows || [])
      .filter((r) => r.counted !== false && r.appliedWeight !== null)
      .map((r) => r.key),
  );
  const prevWeighted = weighted(prevBreakdown);
  const currWeighted = weighted(currBreakdown);
  const axes = AXES.filter((key) => prevWeighted.has(key) !== currWeighted.has(key));
