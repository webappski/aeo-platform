...
/**
 * Sentiment label -> its 0-100 contribution to the sentiment axis.
 *
 * Exported because `run-comparison.js` decomposes the same axis and must value
 * a cell exactly as the index did; a second private copy of this table there
 * would let the two drift and fabricate a tone finding out of the difference.
 * @type {Readonly<Record<string, number>>}
 */
export const SENTIMENT_VALUE = Object.freeze({ positive: 100, neutral: 50, negative: 0 });
 ...
 *
 * Exported for `run-comparison.js`, which must apply the SAME gate when it
 * decomposes the axis — a cell the index excluded but the decomposition
 * averaged in as a real 50 shifts `likeForLike` off zero and invents movement.
 *
 * @param {{label?: string, confidence?: string}|null|undefined} s
 * @returns {boolean}
 */
export function isSignalBearingSentiment(s) {
...
export function computeComponents(latest) {
  // ONE definition of "this cell produced a measurement" (lib/score.js). The
  // filter used to name `error` alone, which was complete only while that was
  // the sole unmeasured outcome. AP-RUNMANUAL-DECLARED-SUBSET added a second —
  // `missing`, a question never put to the engine — and reading it as a cell
  // would put 35 unasked questions into the presence and citation denominators
  // and report a deliberate partial leg as near-total invisibility.
  const results = (latest?.results || []).filter(r => isMeasuredMention(r.mention));
...
/**
 * UVI metric version. Bumped to 2 by AP-UVI-V2-SMALL-SAMPLE-GUARD (2026-09-20):
 * v1 weighted every measured axis regardless of how many cells backed it, so a
 * sentiment averaged over 2 echo cells and a rank taken from 1 cell carried the
 * same authority as a presence share over the whole basket. Published alongside
 * the score (`scores.uviVersion`, `breakdown.uviVersion`) so a v1 baseline is
 * never silently compared against a v2 re-measure.
 */
export const UVI_VERSION = 2;

/**
 * Minimum cells an axis needs before its value may enter the composite.
 *
 * 3 is the floor at which an average stops being a single observation with a
 * decorative mean: at n=1 the axis IS that one cell, at n=2 one flipped cell
 * moves it 50 points. Chosen deliberately low — the guard is there to stop a
 * 1–2 cell axis from dominating a composite, not to demand statistical power we
 * do not have at basket sizes of 15–50 cells.
 */
export const SMALL_SAMPLE_MIN = 3;

/** Which `components` field carries the effective sample size of each axis. */
const AXIS_SAMPLE_KEY = Object.freeze({
  presence: 'sample',
  citation: 'sample',
...
/**
 * Axes the small-sample guard may exclude.
 *
 * Only `sentiment` and `rank` — they are SUB-SAMPLED inside a run (a 21-cell
 * run can produce a sentiment over 2 cells and a rank over 1), which is the
 * exact trap the guard exists for. `presence` and `citation` are denominated
 * over every non-error cell in the run, so their sample IS the run's sample:
 * guarding them on a tiny run would drop every axis, collapse `weightSum` to 0
 * and publish UVI 0 — "invisible" — when the truth is "barely measured". A
 * small run is surfaced as low confidence (`row.lowConfidence`) instead, with
 * the math left alone.
 */
const GUARDABLE_AXES = Object.freeze(['sentiment', 'rank']);

const UVI_KEYS = Object.freeze(['presence', 'sentiment', 'rank', 'citation']);

/**
 * Effective sample size behind an axis, or `null` when the components object
 * does not carry it. `null` means UNKNOWN, not small: a hand-built components
 * object (tests, legacy callers that never went through `computeComponents`)
 * is never guarded on a sample size nobody reported.
 *
 * @param {Object|null|undefined} components
 * @param {'presence'|'sentiment'|'rank'|'citation'} key
 * @returns {number|null}
 */
export function axisSampleSize(components, key) {
  const n = components?.[AXIS_SAMPLE_KEY[key]];
  return typeof n === 'number' && Number.isFinite(n) ? n : null;
}

/**
 * AP-UVI-V2-SMALL-SAMPLE-GUARD — which axes are backed by too few cells to be
 * weighted into the composite.
 *
 * Precedent (WS-CLIENT-4, verawang 2026-07-15, 21 cells): presence 14,
 * citation 0, sentiment 100 over TWO cells, rank 70 over ONE. v1 published
 * UVI 44 — a brand that appeared in 3 of 21 answers read as half-visible,
 * because two echo cells from the brand-probe query carried 45% of the weight.
 * Guarding sentiment and rank leaves presence+citation (0.55 of weight,
 * re-normalised) and the same run scores 9.
 *
 * Pure function of the components object — no I/O, no second pass over cells.
 *
 * @param {Object|null|undefined} components output of `computeComponents`
 * @param {number} [minSample] floor, defaults to `SMALL_SAMPLE_MIN`
 * @returns {Map<string, {key:string, n:number, minSample:number, reason:string}>}
 */
export function smallSampleGuard(components, minSample = SMALL_SAMPLE_MIN) {
  const guarded = new Map();
  for (const key of GUARDABLE_AXES) {
    const v = components?.[key];
    if (v === null || v === undefined) continue; // already excluded as unmeasured
    const n = axisSampleSize(components, key);
    if (n === null) continue;      // sample size unreported — cannot guard on it
    if (n >= minSample) continue;
    guarded.set(key, { key, n, minSample, reason: 'small-sample' });
  }
  return guarded;
}

/**
 * Shared axis resolution for `computeUVI` and `computeUVIBreakdown`.
 *
 * ONE implementation on purpose: these two functions publish the same number on
 * two surfaces (the report's UVI block and the Mission Control payload), and a
 * second copy of the weighting rules is exactly how they drifted before
 * (see the drift note in mc-metadata.js).
 *
 * The guard may NARROW the composite, never empty it — publishing 0 because
 * nothing survived the guard would swap one lie for a louder one. The
 * stand-down below is what enforces that.
 *
 * It cannot fire today, and saying so is the point of this note: the guard only
 * ever considers `GUARDABLE_AXES` (sentiment, rank), while `computeComponents`
 * returns `presence` and `citation` as numbers on every path — 0 when there are
 * no cells at all, never null — so a measured axis the guard cannot touch
 * always survives and the sizes can never match. Read it as a standing
 * precondition for a future `GUARDABLE_AXES`, not as a protection in force:
 * nothing exercises it, so nothing would notice if it broke.
 */
function resolveAxes(components, w, minSample = SMALL_SAMPLE_MIN) {
  const measured = UVI_KEYS.filter(k => {
    const v = components?.[k];
    return v !== null && v !== undefined;
  });

  let guarded = smallSampleGuard(components, minSample);
  if (guarded.size > 0 && guarded.size === measured.length) guarded = new Map();

  const counted = measured.filter(k => !guarded.has(k));
  let weightSum = 0;
  let rawSum = 0;
  for (const k of counted) {
    weightSum += w[k];
    rawSum += components[k] * w[k];
  }
  return { measured: new Set(measured), guarded, counted: new Set(counted), weightSum, rawSum };
}
 ...
 *
 * UVI v2 additionally excludes axes backed by fewer than `SMALL_SAMPLE_MIN`
 * cells (`smallSampleGuard`) and re-normalises over what is left.
 *
 * `minSample` exists for ANALYSIS, not for production: v2 is v1 plus the guard,
 * so passing `0` reproduces the v1 composite exactly — the guard map comes back
 * empty, every measured axis counts, and the arithmetic is the pre-v2 one. That
 * is the only way to read the v1 number of a stored run, because `scores` was
 * computed at render time and never persisted. No caller in `lib/` or `bin/`
 * passes it; the default keeps v2 behaviour for every rendered report.
 */
export function computeUVI(components, weights = DEFAULT_WEIGHTS, minSample = SMALL_SAMPLE_MIN) {
  const w = { ...DEFAULT_WEIGHTS, ...(weights || {}) };
  const { weightSum, rawSum } = resolveAxes(components, w, minSample);
  if (weightSum === 0) return 0;
  return Math.round(rawSum / weightSum);
}
...
export function computeUVIBreakdown(components, weights = DEFAULT_WEIGHTS) {
  ...
  // First pass — which axes are measured AND survive the small-sample guard,
  // so we can compute the re-normalisation denominator BEFORE we render rows.
  // The contribution column shows the re-normalised weight × value, otherwise
  // the row arithmetic would not sum to the headline UVI. Shared with
  // `computeUVI` so the popover can never disagree with the score above it.
  const { measured, guarded, counted, weightSum, rawSum } = resolveAxes(components, w);
  ...
  const rows = keys.map(k => {
    ...
    const isCounted = counted.has(k);
    const guard = guarded.get(k) || null;
    const appliedWeight = isCounted && weightSum > 0 ? w[k] / weightSum : null;
    const contribution = isCounted ? v * appliedWeight : null;
    const axisN = sampleFor(k);
    return {
      key: k,
      label: COMPONENT_META[k].label,
      meaning: COMPONENT_META[k].meaning,
      // A GUARDED axis keeps its value — the reader should see «sentiment 100,
      // n=2, not counted», not a blank. `null` still means "never measured".
      value: present ? v : null,
      sample: axisN,
      weight: w[k],
      appliedWeight,
      contribution,
      // UVI v2 fields. `counted` is what the renderers must branch on:
      // value !== null no longer implies the row entered the composite.
      counted: isCounted,
      lowConfidence: !!guard || (present && axisN.n < SMALL_SAMPLE_MIN),
      excludedReason: guard ? guard.reason : (present ? null : 'not-measured'),
    };
  });

  const uvi = weightSum === 0 ? 0 : Math.round(rawSum / weightSum);
  // `excluded` keeps its v1 meaning — axes with NO underlying measurement.
  // Small-sampled axes are listed separately in `guarded` so consumers that
  // read `excluded` as "this run never measured X" stay correct.
  ...
  return {
    ...
    uvi,
    excluded,
    guarded: Array.from(guarded.values()),
    uviVersion: UVI_VERSION,
  };
}
