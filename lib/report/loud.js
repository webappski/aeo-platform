...
export function indexChart(spec) {
  ...
  const markers = values.map((v, i) => {
    ...
    const cy = round(y(v));
    if (partial[i]) {
      return `<circle class="lr-chart-partial" cx="${cx}" cy="${cy}" r="6"></circle>`
        + `<text class="lr-chart-flag" x="${cx}" y="${round(cy - 12)}" text-anchor="middle">partial</text>`;
    }
  ...
  const every = Math.max(1, spec.labelEvery || 1);
  ...
  return `<svg class="lr-chart" viewBox="0 0 ${W} ${H}" role="img" aria-label="${esc(label)}">
    ...
    <text class="lr-chart-note" x="${LEFT - 16}" y="${round(baseY - 6)}">day 1 · ${esc(String(first))}</text>
    ...
    <text class="lr-chart-last-label" x="${lastX}" y="${round(lastY + 26)}" text-anchor="end">${esc(String(last))}</text>
    ${labels}
  </svg>`;
}
