 ...
 * from the won/held/lost tally BECAUSE OF THIS ENGINE, not an estimate.
 *
 * Answers excluded because only one run asked the question land in the separate
 * `incomparable` bucket and are reported by `buildBasketCaveat` instead. Adding
 * them here would credit an engine's absence with a basket change it had
 * nothing to do with.
 *
 ...
 * scoped to shared cells: `segmentCells` keys on question×provider (the stable
 * question id from `lib/basket-manifest.js`, not the ordinal slot) and routes a
 * one-sided cell to `indeterminate`, or to `incomparable` when it is the
 * question itself that only one run asked. The headline index is not scoped to
 ...
 *   `perplexity` removed from prev  (engine added)        uvi.delta = −20
 *
 * The third number was −17 until UVI v2 (AP-UVI-V2-SMALL-SAMPLE-GUARD,
 * 2026-09-20). Dropping perplexity from `prev` leaves that run's sentiment
 * resting on TWO cells, which v2 reports but no longer weights, so the baseline
 * re-normalises over presence+rank+citation and scores 89 instead of 86. It
 * makes the point of this comment more sharply, not less: a coverage change
 * moves the headline through the composition of each run's own cells.
 *
...
/**
 * The BASKET caveat: a question asked in only one of the two runs.
 *
 * The third axis beside the instrument ("the ruler moved") and the coverage
 * one ("this engine was not on the ruler at all"): here the QUESTION was not
 * in the other run's basket. It is kept separate for the same reason those two
 * are separate — merging them would let a reader discount a rewritten basket as
 * an engine footnote — and it carries one thing neither of the others does:
 * the headline index is scored over each run's OWN questions, so a basket
 * change moves the index by composition even when nothing about the brand's
 * visibility changed.
 *
 * Sizes come from `counts.incomparableQuestions` (questions) and
 * `counts.incomparable` (answers = questions × engines). The two units are kept
 * apart deliberately; a surface that prints one under the other's noun reports
 * a number three times too large.
 *
 * @param {{basketChanged?: boolean, counts?: Object}} model from `buildRunComparison`
 * @returns {{questions: number, cells: number, title: string, sentences: string[]}|null}
 */
export function buildBasketCaveat(model) {
  if (!model?.basketChanged) return null;
  const questions = model.counts?.incomparableQuestions || 0;
  const cells = model.counts?.incomparable || 0;
  if (questions === 0) return null;
  return {
    questions,
    cells,
    title: `${questions} question${questions === 1 ? '' : 's'} ${questions === 1 ? 'was' : 'were'} asked in only one of these two runs`,
    sentences: [
      `${questions === 1 ? 'That question accounts' : 'Those questions account'} for ${cells} answer${cells === 1 ? '' : 's'}, and ${cells === 1 ? 'it is' : 'they are'} left out of the tally of answers won, held and lost — a question measured once cannot have moved.`,
      'The headline index is a different matter: each run is scored over the questions that run asked, so part of its movement is the change of basket rather than a change in how the engines answer about you.',
    ],
  };
}
