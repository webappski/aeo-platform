 ...
 *    `extractionSources` and `degraded` (which grading model failed on that
 *    cell, and why). Never spread a segment entry. Every field below
...
export function buildComparisonPayload(snapshots, opts = {}) {
  ...
  const out = {
    ...
    uvi: null,
    weightBasis: null,
    // The instrument axis. Null on a first run / no pair, exactly like the
    // fields around it; `{changed: false}` once a pair exists and the engines
    // held. The portal needs it for the same reason the markdown report does:
    // it renders "who took your place", and that story is only true when both
    // runs asked the same model.
    modelChange: null,
    // The coverage axis, alongside the instrument one. An engine measured in
    // only one of the two runs takes its cells out of every lost/held/gained
    // figure the portal renders, and until 2026-09-02 nothing anywhere said so.
    coverageChange: null,
    // The basket axis, alongside the instrument and coverage ones. A QUESTION
    // asked in only one of the two runs takes its cells out of every
    // lost/held/gained figure — and, unlike those two, it also moves `uvi.delta`
    // by composition alone, because each run's index is scored over its own
    // questions. The portal renders that delta, so it has to be told.
    basketChange: null,
    counts: null,
    lost: null,
    gained: null,
  ...
  out.modelChange = {
    changed: model.modelChanges.changedProviders.length > 0,
    providers: model.modelChanges.changedProviders,
    // Only the kinds that can explain movement — a dated-snapshot pin is
    // provenance, not a caveat, and shipping it as one would train the portal's
    // readers to ignore the flag.
    explanatoryProviders: model.modelChanges.explanatoryProviders,
    noLikeForLikeProviders: model.modelChanges.noOverlapProviders,
    engines: model.modelChanges.entries.map((e) => ({
      provider: e.provider,
      before: e.before,
      after: e.after,
      kind: e.kind,
      strength: e.strength,
      hasLikeForLike: e.hasLikeForLike,
    })),
  };
  out.coverageChange = model.coverageChange
    ? {
      dropped: model.coverageChange.dropped.map((d) => ({ ...d })),
      added: model.coverageChange.added.map((a) => ({ ...a })),
    }
    : null;
  // `counts.never` is CELL-level (one entry per query×engine, matching
  // `lost`/`gained`), but `neverHeld` below is QUERY-level (one entry per
  // blank query, capped at NEVER_HELD_LIMIT) — the two are different units
  // and must never be compared directly. `neverQueries` is the TRUE total
  // query count, before the cap, for a like-for-like truncation check.
  out.counts = { ...model.counts, neverQueries: model.blankQueries.length };

  // Basket axis. `questions` is QUERY-level, `cells` is query×engine — the same
  // unit split `counts.never` / `neverQueries` carries above, and for the same
  // reason: a consumer that compares the two as one unit reports a number three
  // times too large.
  out.basketChange = {
    changed: !!model.basketChanged,
    questions: numOrNull(model.counts.incomparableQuestions) ?? 0,
    cells: numOrNull(model.counts.incomparable) ?? 0,
  };
