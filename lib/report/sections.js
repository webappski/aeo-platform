...
export function sectionActionableGaps(snapshots, opts = {}) {
  ...
  const rows = prioritised.map(({ r, verified, unverified }) => {
    ...
    let action;
    if (unverifiedOnly) {
      // Only one extractor model flagged competitors here — the other returned
      // empty. The cross-check tier is exactly for this: do not recommend an
      // outreach move on a single-model signal. Surface the row for visibility
      // but flip the action to a diagnostic note instead of a «Pitch …» step.
      action = `Cross-check this cell — only one extractor model flagged competitors here, the other returned empty. Likely retailers / customers / unrelated brands picked up in error.`;
    } else if (cellHost) {
...
export function sectionUnifiedVisibilityIndex(snapshots) {
  ...
  const popover = renderUVIBreakdownPopover(breakdown);
