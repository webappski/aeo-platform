...
function rcLostBlock(model) {
  ...
  const lines = rows.map((entry) => {
    ...
    const swapped = entry.engineChanged ? ' _(different model than last run — see below)_' : '';
...
/** "ChatGPT ran gpt-5-search-api last time and gpt-5.4-mini this time." */
function rcEngineTransitions(model) {
  const driver = model.driverSummary.engineChange;
  if (!driver) return '';
  return driver.transitions.map((t) => {
    const [before, after] = t.text.split(' → ');
    return `${providerLabel(t.provider)} ran ${rcEngineName(before)} last time and ${rcEngineName(after)} this time.`;
  }).join(' ');
}
 ...
 * is the whole point of naming it (see the tone-contract exception above).
 */
function rcEngineName(id) {
...
function rcEngineChangeCaveat(model) {
  ...
  const parts = [`**A note on the measurement:**${alreadyNamed ? '' : ` ${rcEngineTransitions(model)}`}`];
  // WHAT the change was, in the caller's own words. Branching on strength alone
  // was wrong: `moderate` covers both a genuine small step and "the two ids
  // could not be related at all", and the small-step wording asserted a
  // smallness nobody had measured. It also mislabelled a switch between pasted
  // and API answers as "a smaller step within the same model family".
  parts.push(RC_ENGINE_CHANGE_EXPLANATION[driver.strongestKind] || RC_ENGINE_CHANGE_EXPLANATION.unknown);
  const blind = driver.noLikeForLike.map(providerLabel);
  if (blind.length > 0) {
    parts.push(`On ${joinWithAnd(blind)} there is no overlap at all between the two runs — not one answer was measured the same way twice, so nothing on ${blind.length === 1 ? 'that engine' : 'those engines'} is a like-for-like comparison.`);
  }
  ...
  // movement is still real" sits two paragraphs under a headline reading
  // "unchanged" — a self-contradiction on the one surface this whole section
  // exists to keep honest, and one that only appears BECAUSE an engine swap now
  // (correctly) stops the calm "held steady" message from firing.
  const moved = model.counts.lost > 0 || model.counts.gained > 0
    || model.driverSummary.hasGenuineConditionalChange;
  parts.push(moved
    ? 'So some of the movement above belongs to that change rather than to your visibility. It is still real and still worth acting on; it is the size of it that carries this caveat.'
 ...
 *
 * Movement is claimed exactly once, by the closing sentence above, which knows
 * whether any occurred. These used to end in "…so some of the movement above
 * belongs to that change", which put the assertion in two places — and the
 * second one fired on a flat run, printing it under a headline reading
 * "unchanged".
 *
 ...
 */
const RC_ENGINE_CHANGE_EXPLANATION = {
  ...
  // already spells out "a pasted answer from its own app" (rcEngineName).
