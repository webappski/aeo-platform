...
import { computeComponents, computeUVI, computeUVIBreakdown, computeDiscoverability, SMALL_SAMPLE_MIN } from './visibility-index.js';
import { buildRunComparison } from './run-comparison.js';
import { buildDeclaredSubsetCaveat } from '../declared-subset.js';
import { headlineComparability } from '../run-axes.js';
import { buildMentionRoleLine } from './mention-role.js';
import { buildSectionDegradationCaveat } from './section-degradation.js';
import {
...
import {
  providerLabel, joinWithAnd, buildInstrumentCaveat, buildCoverageCaveat,
  instrumentTransitions, INSTRUMENT_ROW_NOTE,
} from './instrument-caveat.js';
...
import { sliceStats, aggregateScore, isMeasuredMention } from '../score.js';
...
// Engine names and the instrument/coverage caveat copy live in
// `instrument-caveat.js` so the HTML report renders the same words rather than
// its own paraphrase. Re-exported here because `providerLabel` has been this
// module's public name for it since v0.3.
...
/** Per-provider hit ratio. Returns { hits, total, rate } — rate as a 0..1
 *  fraction (this caller's historical contract), counts from the ONE
 *  denominator in lib/score.js. */
function providerStats(results, provider) {
  const rs = results.filter(r => r.provider === provider);
  const st = sliceStats(rs);
  return { hits: st.hits, total: st.valid, rate: st.valid > 0 ? st.hits / st.valid : 0 };
}
...
export function sectionHero(snapshots) {
  ...
  // AP-DIFF-ENGINE-MIX-AXIS — this header never went through lib/diff.js and
  // subtracted the two headlines directly, so the engine-set gate the diff
  // applies did not protect the most-read number in the report. On the real
  // 2026-08-31 (3 engines) → 2026-09-16 (2 engines) pair it printed
  // «▼ −2pp vs 2026-08-31» for a pair whose shared cells did not move at all.
  const headline = prev ? headlineComparability(prev, latest) : null;
  const scoreDelta = prev && headline.comparable ? latest.score - prev.score : null;
  const trendMarker = !prev
    ? '▪ BASELINE'
    : !headline.comparable
      ? `▪ not compared to ${prev.date} — ${headline.engines.countBaseline} engine${headline.engines.countBaseline === 1 ? '' : 's'} measured then, ${headline.engines.countNew} now`
      : scoreDelta > 0 ? `▲ +${scoreDelta}pp vs ${prev.date}`
      : scoreDelta < 0 ? `▼ ${scoreDelta}pp vs ${prev.date}`
      : '▪ no change';
  // The withheld delta needs its reason ON the page, not only in the diff — a
  // reader who sees no arrow must not conclude the score stood still.
  const trendNote = headline && !headline.comparable && headline.note
    ? `\n_${escMd(headline.note)}_\n`
    : '';
...
export function sectionKeyMetrics(snapshots) {
  ...
  const tl = trafficLight(latest.score);
  // Same gate as the hero (AP-DIFF-ENGINE-MIX-AXIS): the per-provider deltas
  // below already refuse to compare an engine the baseline never measured, but
  // the OVERALL card subtracted two percentages computed over different engine
  // sets and called the difference a movement.
  const headlineKM = prev ? headlineComparability(prev, latest) : null;
  const scoreDelta = prev && headlineKM.comparable ? latest.score - prev.score : null;
  const overallDelta = !prev
    ? '▪ baseline'
    : !headlineKM.comparable
      ? '▪ engines changed'
...
`;
  ...
  // Pair cells by the QUESTION, not by its ordinal label.
  //
  // This table used to look the previous run up with
  // `p.query === r.query && p.provider === r.provider`, where `query` is the
  // label `Q12` — a position in whichever basket that run used. On the real
  // 2026-08-27 → 2026-08-31 pair (56 questions vs 50, sharing 31) that produced
  // 20 green "no → yes" rows whose two sides were two DIFFERENT questions, in a
  // table that ships inside the white-label client document
  // (`renderWhiteLabelMarkdown` → this section). Identity now comes from
  // `lib/basket-manifest.js`, exactly as in `lib/diff.js`, and questions only
  // one run asked are counted and named below instead of being paired.
  const mIdx = manifestIndex(curr.basketManifest || prev.basketManifest || null);
  const prevByKey = new Map();
  const prevQuestions = new Set();
  for (const p of prev.results || []) {
    const ident = resolveQueryIdentity(p, mIdx);
    prevByKey.set(`${ident.id}|${p.provider}`, p);
    prevQuestions.add(ident.id);
  }

  const newQuestions = new Set();
  const seenQuestions = new Set();
  for (const r of curr.results) {
    const ident = resolveQueryIdentity(r, mIdx);
    seenQuestions.add(ident.id);
    if (!prevQuestions.has(ident.id)) { newQuestions.add(ident.id); continue; }
    const pr = prevByKey.get(`${ident.id}|${r.provider}`);
    ...
    if (pr.mention !== r.mention) {
      ...
      changes.push({
        ...
        queryText: r.queryText || ident.text || '',
        market: ident.market || null,
        was: pr.mention, now: r.mention,
        note: methodChanged ? `mixed-method (${pr.source || 'api'} → ${r.source || 'api'})` : '',
      });
    }
  }
  const retiredQuestions = [...prevQuestions].filter(id => !seenQuestions.has(id));

  // One sentence, in the client's document, whenever the two runs did not ask
  // the same questions. Without it the table reads as a complete account of the
  // period, which it is not.
  const basketNote = (newQuestions.size > 0 || retiredQuestions.length > 0)
    ? `\n_The two runs did not ask the same questions: ${newQuestions.size} asked only on ${curr.date} (no baseline to compare with) and ${retiredQuestions.length} asked only on ${prev.date}. Those are excluded from this table — a question measured once cannot have moved._\n`
    : '';
...
${basketNote}`;
  ...
  const rows = changes.map(ch => {
    ...
    // The question TEXT, with the ordinal label beside it. The label alone is a
    // position in this run's basket and means nothing to a reader comparing two
    // runs; the text is what was actually asked.
    const qCell = ch.queryText
      ? `${escMd(ch.query)} — ${escMd(ch.queryText.slice(0, 70))}${ch.market ? ` _(${escMd(ch.market)})_` : ''}`
      : escMd(ch.query);
    return `| ${deltaArrow({ value: sign })} | ${providerLabel(ch.provider)} | ${qCell} | ${ch.was} | ${ch.now}${noteCell} |`;
  });
...
_Only cells covered by BOTH runs — and only questions BOTH runs asked — are listed. Cells added or removed by config changes between runs are excluded._
${basketNote}`;
}
...
//     «compositional», sample basis) reaches the rendered copy;
//   - ONE EXCEPTION, added 2026-09-01: a MODEL ID is printed when the engine
//     changed model between the two runs. The names look internal and are not:
//     they are the only form in which "the ruler was swapped" can be checked
//     rather than taken on faith, and the founder's ruling is that the movement
//     is always shown and the swap always explained. A softened «the engine was
//     updated» would be the same unfalsifiable sentence the report exists to
//     replace. Model names appear ONLY in that caveat, nowhere else in the copy.
...
function rcLostBlock(model) {
  ...
  const lines = rows.map((entry) => {
    ...
    // Per-ROW, not per-section. This table is where the report is at its most
    // assertive — it names companies as having taken specific ground — and on
    // the run that motivated this, three of its five rows came from an engine
    // that had answered a different question on a different model. A footnote
    // under the table does not reach a reader who has already read the names.
    const swapped = entry.engineChanged ? ` _(${INSTRUMENT_ROW_NOTE} — see below)_` : '';
    return `| ${escMd(entry.queryText)} | ${providerLabel(entry.provider)} | ${who}${flag}${swapped} |`;
  });
  const changed = model.counts.lostOnChangedEngine || 0;
  const caveat = changed > 0
    ? `\n_${changed} of ${rows.length} ${changed === 1 ? 'row' : 'rows'} above ${changed === 1 ? 'was' : 'were'} measured on a different model than last time — ` +
      `on those, part of what looks like a competitor taking your place is the engine itself answering differently. ` +
      `${instrumentTransitions(model)}_\n`
    : '';
...
/**
 * The engine-change paragraph: what moved, and how much of the movement the
 * swap can account for. Always ATTRIBUTION — the delta above it is stated in
 * full, exactly as on a run where nothing changed (founder ruling 2026-09-01:
 * a customer is never told their own movement cannot be compared).
 *
 * The sentences come from `buildInstrumentCaveat` and are shared verbatim with
 * the HTML report; this function only decides the markdown wrapper and which of
 * them this surface has already said elsewhere.
 */
function rcEngineChangeCaveat(model) {
  const caveat = buildInstrumentCaveat(model);
  if (!caveat) return '';
  // The "was → is" pair is stated ONCE per section. It belongs inside the lost
  // table when there is one — that is where the misleading story gets told — and
  // moves here only when there is no such table to carry it.
  const alreadyNamed = model.segments.lost.length > 0 && (model.counts.lostOnChangedEngine || 0) > 0;
  const parts = [`**A note on the measurement:**${alreadyNamed ? '' : ` ${caveat.transitions}`}`];
  parts.push(caveat.explanation);
  if (caveat.noLikeForLike) parts.push(caveat.noLikeForLike);
  parts.push(caveat.closing);
  return `_${parts.join(' ')}_`;
}

/**
 * The coverage paragraph: an engine that was on only one side of the pair.
 *
 * Separate from the instrument caveat above on purpose — "the ruler moved" and
 * "this engine was not on the ruler at all" are different facts, and only the
 * second means part of the comparison has no basis rather than an uncertain
 * one. Both can fire on the same run.
 */
function rcCoverageChangeNote(model) {
  const caveat = buildCoverageCaveat(model);
  if (!caveat) return '';
  return `_**A note on the coverage:** ${caveat.sentences.join(' ')}_`;
}

/**
 * The basket itself changed between the two runs — say it, sized, next to the
 * figures it qualifies.
 *
 * This is the counterpart of `rcCoverageChangeNote` on the other axis: that one
 * reports an ENGINE measured in only one run, this one a QUESTION asked in only
 * one run. Both mean a part of the comparison has no second side. The headline
 * index is not scoped to the shared questions — each run is scored over its own
 * results — so a delta across a changed basket is partly composition, and the
 * only honest thing to do beside it is name the size of the change.
 */
function rcBasketChangeNote(model) {
  if (!model.basketChanged) return '';
  const q = model.counts.incomparableQuestions || 0;
  const cells = model.counts.incomparable || 0;
  return `_**A note on the questions:** the two runs did not ask the same set — ${q} question${q === 1 ? '' : 's'} (${cells} answer${cells === 1 ? '' : 's'}) ${q === 1 ? 'was' : 'were'} asked in only one of them. Those are excluded from the won/held/lost figures above, because a question measured once cannot have moved. The headline index is scored over each run's own questions, so part of its movement is the change of basket rather than a change in visibility._`;
}
...
function rcReplacementsLine(model) {
  ...
  // This line is the most quotable sentence in the section and the easiest to
  // act on wrongly: it reads as a competitive ranking. It is built only from
  // lost cells, so when most of those were measured on a swapped engine the
  // list is partly a list of who that OTHER model happens to name.
  const changed = model.counts.lostOnChangedEngine || 0;
  const qualifier = changed > 0
    ? ` _(${changed === model.counts.lost ? 'every' : `${changed} of ${model.counts.lost}`} lost ${model.counts.lost === 1 ? 'answer' : 'answers'} came from an engine running a different model than last time — treat this as a list to check, not a confirmed ranking)_`
    : '';
  return `_Most frequently filling the gap: ${top}._${qualifier}`;
}
...
`;
  ...
  // An engine swap also disqualifies the calm message: "visibility held steady"
  // is a claim about a stable measurement, and it isn't one if the ruler moved.
  // Holding ground ACROSS a model change is a genuinely good sign — but it has
  // to be said, not silently compressed into "nothing happened".
  // A coverage change disqualifies it for a blunter reason: "across every
  // tracked question and engine" is false the moment an engine went unmeasured,
  // and its cells land in `indeterminate`, which keeps lost and gained at zero —
  // so the calm message is exactly what a dropped engine used to produce.
  // A basket change disqualifies it for the same blunt reason a coverage change
  // does, and more so: with questions asked in only one of the two runs, "across
  // every tracked question" is false, and those cells now sit in `incomparable`,
  // which keeps lost and gained at zero — so without this clause a basket
  // rewrite would render as "visibility held steady".
  const stable = model.counts.lost === 0 && model.counts.gained === 0
    && !model.driverSummary.hasGenuineConditionalChange
    && !model.driverSummary.engineChange
...
`;
  ...
  const body = [
    ...
    rcEngineChangeCaveat(model),
    rcCoverageChangeNote(model),
    rcBasketChangeNote(model),
...
export function sectionTrend(snapshots) {
  ...
  // One line per QUESTION, not per ordinal slot. A sparkline is a claim that
  // the same question was measured N times; keyed on the label `Q12` it drew a
  // continuous line across whichever questions happened to occupy slot 12 in
  // each basket. Runs that did not ask a question contribute `null`, which the
  // sparkline already renders as a gap rather than as a zero.
  const trendIdx = manifestIndex(
    snapshots[snapshots.length - 1]?.basketManifest || null,
  );
  const identityOf = (r) => resolveQueryIdentity(r, trendIdx).id;
  const latest = snapshots[snapshots.length - 1];

  const questions = new Map();   // id → { id, label, text }
  for (const s of snapshots) {
    for (const r of s.results || []) {
      const id = identityOf(r);
      if (!questions.has(id)) questions.set(id, { id, label: r.query, text: r.queryText || r.query });
    }
  }
  // Latest run's naming wins for a question both runs asked, so the list reads
  // in the current report's own vocabulary.
  for (const r of latest.results || []) {
    const q = questions.get(identityOf(r));
    if (q) { q.label = r.query; q.text = r.queryText || r.query; }
  }

  const ordered = [...questions.values()].sort((a, b) => a.label.localeCompare(b.label, undefined, { numeric: true }));
  const lines = ordered.map(q => {
    const values = snapshots.map(s => {
      const rs = (s.results || []).filter(r => identityOf(r) === q.id);
      if (rs.length === 0) return null;  // this run did not ask it — a gap, not a zero
      const st = sliceStats(rs);
      if (st.valid === 0) return null;   // nothing measured that day — a gap, not a zero
      return st.rate;
    });
    const sp = sparkline({ values });
    return `- ${sp} **${escMd(q.label)}:** ${escMd(q.text)}`;
  });
  ...
  const matrix = queries.map(q => {
    ...
    return {
      ...
      cells: providers.map(p => {
        ...
        // A call that FAILED and a question nobody ASKED both render as a dash,
        // but they are not the same fact and must not share a status string:
        // `mention: 'missing'` became a real value with the declared-subset
        // import, so 'missing' can no longer double as the label for an error.
        if (!r) return { status: 'missing', competitors: [] };
        if (r.mention === 'error') return { status: 'error', competitors: [] };
        const cited = (r.competitors || []).slice(0, 4);
        // A GAP is a question this engine answered without naming us. A cell
        // nobody asked is not a gap — counting it published «4 gaps found» over
        // a run with two, in the client's own document. Same class as the four
        // denominators AP-RUNMANUAL-DECLARED-SUBSET fixed; this prose line was
        // the fifth surface and read «not a hit» as «a miss».
        if (isMeasuredMention(r.mention) && r.mention !== 'yes' && r.mention !== 'src') totalGaps++;
 ...
 * @param {Object}  [opts]
 * @param {boolean} [opts.whiteLabel]  the legend keeps the low-confidence flag
 *        it has to keep and drops the two-classifier-model architecture behind
 *        it — same class as the degradation caveat
 *        (lib/report/section-degradation.js).
 */
export function sectionSentiment(snapshots, opts = {}) {
  const latest = snapshots[snapshots.length - 1];
  ...
  const method = opts.whiteLabel === true
    ? 'Sentiment is scored per mention — an uncertain score is kept but flagged as low-confidence.'
    : 'Sentiment is cross-checked by two classifier models — disagreements degrade to "neutral" with a low-confidence flag.';

  return `## How AI Engines Portray Your Brand

_${summary} across ${total} mention${total !== 1 ? 's' : ''}. ${method}_
...
// ─── Section: Intent Breakdown (NEW v0.4) ───
//
// Visibility aggregated by the `tag` on each query in .aeo-tracker.json.
// `init` writes the intent class it already computed (comparison / problem /
// commercial / informational / vertical — lib/init/research/classify-intent.js);
// a hand-edited tag is honoured as-is, so a user is free to segment by region
// or language instead. Hidden when no tags are defined — zero impact on
// existing configs.
//
// Up to 1.15.x this section had never rendered in any run: nothing wrote a tag
// into a config, so `r.tag` was always absent and the filter below always
// emptied. Anything that "worked" here was untested by construction — treat
// changes accordingly.

export function sectionFunnelBreakdown(snapshots, { whiteLabel = false } = {}) {
  const latest = snapshots[snapshots.length - 1];
  const tagged = (latest.results || []).filter(r => !!r.tag);
  if (tagged.length === 0) return '';

  // A breakdown needs something to break down. With one class the table is a
  // single row at 100% — it looks like a measurement and carries no information,
  // because "all of your questions are commercial" is a fact about the basket,
  // not about visibility. That is a reachable state, not a hypothetical: when
  // the site language is one the classifier cannot read, every question keeps
  // the brainstorm tag, and brainstorm only ever emits `commercial`. Hidden in
  // every mode — markdown, white-label and HTML alike — for the same reason it
  // is hidden when nothing is tagged at all.
  if (new Set(tagged.map(r => r.tag)).size < 2) return '';

  // One bucket per tag, counted through the ONE denominator (lib/score.js).
  // This table used to divide by every cell carrying the tag — errors included
  // — so a tag whose calls failed was rendered as a tag the engines ignored.
  const byTagRows = new Map();
  for (const r of tagged) {
    if (!byTagRows.has(r.tag)) byTagRows.set(r.tag, []);
    byTagRows.get(r.tag).push(r);
  }
  const byTag = new Map();
  for (const [tag, rows] of byTagRows) {
    const st = sliceStats(rows);
    byTag.set(tag, { tag, total: st.valid, mentions: st.hits, errors: st.errors });
  }
  ...
  // ONE builder, two wordings — the same shape `sectionSentiment` and the
  // degradation caveat use. The default blurb names our config file and reads
  // the table FOR the client ("means AI knows your category but not why to
  // choose you"). Both are wrong in a white-label deliverable: it ships under
  // the legend of a self-made local statistics tool, so it must not name the
  // tool, and it carries statistics rather than advice. The table itself stays
  // in both — withholding the figures would make the document look broken
  // rather than reserved.
  //
  // This leak already existed in the markdown white-label renderer and had
  // never fired, because no run has ever carried a tag. It is closed here
  // before the HTML page starts rendering the section and makes it reachable.
  //
  // Both wordings used to describe a FUNNEL. They named stages the tool has
  // never measured: what `init` classifies is the intent of each question
  // (comparison / problem / commercial / informational / vertical), and the
  // default blurb went further and read a ToFu→BoFu story out of it. Mapping
  // intents onto a funnel model is our judgement about someone's market, and
  // printing it beside real percentages presents that judgement as measurement
  // (founder ruling 2026-09-21). The section now says what it counts.
  const blurb = whiteLabel
    ? '_Visibility split across the intent classes defined for this question set._'
    : '_Visibility split across the tags in `.aeo-tracker.json` — the intent class of each question. A gap between classes tells you WHICH kind of question loses you: strong on `informational` but weak on `comparison` means AI explains your category without putting you in the shortlist._';

  return `## Visibility by Intent
...
export function sectionActionableGaps(snapshots, opts = {}) {
  ...
  const gaps = results.filter(r => {
    ...
    // `isMeasuredMention` (lib/score.js), not a bare error check: a question a
    // declared subsample never asked is not a displacement gap — nobody was
    // named instead of us there, because nobody was asked.
    if (!isMeasuredMention(r.mention)) return false;
...
export function sectionGeoComparison(snapshots) {
  ...
  const cellRate = (region, provider) => {
    const cells = withRegion.filter(r => r.region === region && r.provider === provider);
    const st = sliceStats(cells);
    if (st.valid === 0) return null;   // nothing measured in this region/engine pair
    return { hits: st.hits, total: st.valid, rate: st.rate };
  };
  ...
  // Same denominator as the per-cell fractions in the table above — a footer
  // total that doesn't add up to its own rows is how a report loses a reader.
  const regionTotals = sliceStats(withRegion);
  const totalCells = regionTotals.valid;
  const totalHits = regionTotals.hits;
...
export function renderUVIBreakdownPopover(breakdown, opts = {}) {
  ...
  const rows = breakdown.rows.map(r => {
    ...
    // UVI v2 — a MEASURED axis can still be excluded from the composite when
    // too few cells back it (small-sample guard). Show the value, say how many
    // cells it rests on, and mark plainly that it did not move the score.
    if (r.counted === false) {
      return `<tr><td><strong>${r.label}</strong><div class="uvi-meaning">${r.meaning}</div></td>` +
             `<td class="uvi-num">${r.value}<div class="uvi-sub">${formatSample(r)} · low confidence</div></td>` +
             `<td class="uvi-num">—<div class="uvi-sub">not counted (n &lt; ${SMALL_SAMPLE_MIN})</div></td>` +
             `<td class="uvi-num">—</td></tr>`;
    }
  ...
  //
  // The redistribution list must be built from the COUNTED rows, not from
  // "value !== null": under UVI v2 a measured-but-guarded axis has a value and
  // no applied weight, and listing it here would print NaN% as its new weight.
  const labelWithWeight = (k) => {
    const meta = breakdown.rows.find(r => r.key === k);
    return `<strong>${meta.label}</strong> (${(meta.weight * 100).toFixed(0)}% weight)`;
  };
  const guardedAxes = breakdown.guarded || [];
  let renormBanner = '';
  if (breakdown.excluded.length > 0 || guardedAxes.length > 0) {
    const redistTo = breakdown.rows
      .filter(r => r.counted !== false && r.appliedWeight !== null)
      .map(r => `${r.label} ${(r.weight * 100).toFixed(0)}% → ${fmtPct(r.appliedWeight)}`)
      .join(', ');
    const parts = [];
    if (breakdown.excluded.length > 0) {
      parts.push(`${breakdown.excluded.map(labelWithWeight).join(', ')} not measured this run`);
    }
    if (guardedAxes.length > 0) {
      parts.push(guardedAxes
        .map(g => `${labelWithWeight(g.key)} measured over only ${g.n} cell${g.n === 1 ? '' : 's'} — below the ${g.minSample}-cell floor, so it is reported but not counted`)
        .join('; '));
    }
    renormBanner = `<p class="uvi-renorm">${parts.join('; ')} — that weight is redistributed proportionally across the other components (${redistTo}).</p>`;
  }
...
export function sectionUnifiedVisibilityIndex(snapshots, publicMode = false) {
  ...
  // UVI v2 — which axes actually entered the composite. A measured axis backed
  // by fewer than SMALL_SAMPLE_MIN cells is reported with its value but marked
  // «not counted», so the table can never imply a 1-cell rank moved the score.
  const notCounted = new Set((breakdown.guarded || []).map(g => g.key));

  const componentRow = (key, label, value, weight, hint) => {
    ...
    if (notCounted.has(key)) {
      const g = (breakdown.guarded || []).find(x => x.key === key);
      return `| **${label}** | <span class="rate-text" data-tone="muted">${value}/100</span> | — | <span class="share-bar" data-tone="muted" style="--bar-w:2%"></span> | ${hint} — only ${g.n} cell${g.n === 1 ? '' : 's'}, below the ${g.minSample}-cell floor, so it is not counted in the index |`;
    }
  ...
  const rows = [
    componentRow('presence',  'Presence',  c.presence,  0.35, presenceHint),
    componentRow('sentiment', 'Sentiment', c.sentiment, 0.25, sentimentHint),
    componentRow('rank',      'Rank',      c.rank,      0.20, rankHint),
    componentRow('citation',  'Citation',  c.citation,  0.20, 'share of cells with brand domain in citations'),
  ].join('\n');
...
export function segmentByBrandFit(results) {
  const rows = (Array.isArray(results) ? results : []).filter(r => r && typeof r.brandFit === 'string' && r.brandFit);
  if (rows.length === 0) return {};
  const byFit = new Map();
  for (const r of rows) {
    const key = r.brandFit.toLowerCase();
    if (!byFit.has(key)) byFit.set(key, []);
    byFit.get(key).push(r);
  }
  const acc = {};
  for (const [key, list] of byFit) {
    const st = sliceStats(list);          // ONE denominator — lib/score.js
    if (st.valid === 0) continue;         // a bucket where every call errored says nothing
    acc[key] = { total: st.valid, mentions: st.hits, rate: st.rate };
  }
  return acc;
}

export function sectionScoreRepresentativeness(snapshots, opts = {}) {
  const latest = snapshots[snapshots.length - 1];
  if (!latest || !Array.isArray(latest.results) || latest.results.length === 0) return '';

  // Representativeness is about how many cells actually MEASURED something —
  // unasked cells from a declared subsample must not pad n (lib/score.js).
  const cells = latest.results.filter(r => isMeasuredMention(r.mention));
  const n = cells.length;
  if (n === 0) return '';

  const blocks = [];

  // ── Block 0: declared subsample (AP-RUNMANUAL-DECLARED-SUBSET) ──
  // A leg imported with `--declared-subset` deliberately answered part of the
  // basket. That belongs FIRST in the representativeness section, ahead of the
  // small-N and coverage notes: it is the one reason a reader may be looking at
  // an engine column with a denominator different from the others, and nothing
  // else on the page says so.
  for (const stamp of Object.values(latest.declaredSubsets || {})) {
    const caveat = buildDeclaredSubsetCaveat(stamp);
    if (!caveat) continue;
    blocks.push(`> **${escMd(caveat.title)}.** ${caveat.sentences.map(escMd).join(' ')}`);
  }

  // ── Block 0b: named vs recommended (AP-MENTION-ROLE-VENDOR-VS-SOURCE) ──
  // The headline counts every answer that named the brand. Some of those
  // answers named it as the AUTHOR of a ranking about rival suppliers, which
  // raises the count while sending the buyer elsewhere. Rendered only when the
  // run actually classified roles — «0 recommended» on an unclassified run
  // would state a finding nobody made.
  const roleLine = buildMentionRoleLine(latest.results);
  if (roleLine) blocks.push(`> ${roleLine}`);

  // ── Block 0c: what this run could not classify (AP-FAILBRANCH-TAIL2 #2) ──
  // A lost classify-tier call silently thins the competitor and sentiment
  // sections; "only two competitors this week" then reads as a finding about
  // the market. The sentences come from lib/report/section-degradation.js and
  // are the SAME ones the HTML report prints — a caveat two surfaces word
  // differently is a caveat a reader can shop around.
  const degradation = buildSectionDegradationCaveat(latest.results, { whiteLabel: opts.whiteLabel === true });
  if (degradation) {
    blocks.push(`> **${escMd(degradation.title)}.** ${degradation.sentences.map(escMd).join(' ')}`);
  }
...
export function sectionRunVerdict(snapshots, opts = {}) {
  ...
  // No denominator: `fig` then renders the `points` unit as a percentage, which
  // is what this metric is. Passing 100 printed "14 of 100" — index phrasing
  // for a share, one line above the count it is the share OF.
  lines.push(fig(M.index));
  ...
  lines.push(`- **Cited without being named:** ${lift.cited} of ${Number.isFinite(lift.valid) ? lift.valid : lift.total}`);
  lines.push('');
