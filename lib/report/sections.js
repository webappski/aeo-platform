...
const ENGINE_FALLBACK_TIPS = {
  openai: {
    why: 'ChatGPT search retrieves via OpenAI’s own crawl (OAI-SearchBot) and Google-aligned SERP data (Bing dependence ended Aug 2025). Review platforms and community Q&A remain among its highest-weight sources.',
    tips: [
  ...
  gemini: {
    why: 'Gemini grounds responses with Google’s FastSearch semantic retrieval, not full Search ranking — passage-level relevance and entity/Knowledge-Graph footprint outweigh raw backlink authority here.',
    tips: [
...
_Source: \`${audit.robots.url}\` (HTTP ${audit.robots.status ?? 'n/a'}). Re-audit on every \`aeo-platform report\`._
`;
}
...
export function sectionScoreRepresentativeness(snapshots) {
  ...
  // Only renders when results carry a brandFit label. WIRING NOTE: the label is
  // computed at init (candidate research) but the active basket is persisted as
  // bare query strings, so today's results have no brandFit field and this block
  // is skipped (never fabricated). The read path here + the result-attach in
  // bin/aeo-tracker.js (queryBrandFits) are additive and ready; populating the
  // saved basket with the label is gated on a founder decision because it means
  // writing structured query objects into config.queries, which collides with
  // the string-only contract in basket-history.js / mergeQueries / suggest.js.
...
export function sectionFooter(snapshots, lang = 'en') {
  const latest = snapshots[snapshots.length - 1];
...
_Generated by aeo-platform. Raw responses: \`aeo-responses/${escMd(latest.date)}/\`. Re-run: \`aeo-platform report\`._
`;
}
