...
// One error class for "the model's answer could not be read", shared with the
// research loops that already retry on exactly this and nothing else.
import { LlmParseError } from '../init/research/parse-error.js';
import { runWithParseRetry } from './classify-retry.js';
...
export function parseExtractorResponse(raw) {
  if (!raw || typeof raw !== 'string') {
    throw new LlmParseError('extractor returned empty response');
  }
  ...
  try {
    ...
    if (!m) throw new LlmParseError('extractor response is not JSON and contains no {...} block');
    try { parsed = JSON.parse(m[0]); }
    catch (err) { throw new LlmParseError(`extractor response unparseable: ${err.message}`); }
  }

  if (!parsed || typeof parsed !== 'object' || !Array.isArray(parsed.brands)) {
    throw new LlmParseError('extractor JSON missing "brands" array');
  }
...
export async function extractWithTwoModels({
  ...
  // An un-parseable body gets exactly one re-ask (see classify-retry.js for who
  // answers it and why nothing else is retried here). Without `fallbacks` the
  // re-ask goes back to the same model; with none configured the outcome is the
  // pre-1.15 one, minus the lost slot.
  const runOne = async (p) => {
    const r = await runWithParseRetry({
      slot: p,
      fallbacks,
      attempt: async (provider) => {
        const out = await extractWithSingleModel({
          text, brand, domain, category,
        ...
        return { value: out.brands, costInfo: out.costInfo };
      },
    });
    if (r.ok) {
      return {
        ok: true, brands: r.value, costInfo: r.costInfo,
        ...(r.retriedOn ? { retriedOn: r.retriedOn } : {}),
      };
    }
    return {
      ok: false, brands: [], costInfo: null,
      error: r.error,
  ...
  // Single-key mode (1.1.8): secondary may be null — run primary alone. The
  // merge below then routes every brand to `unverified`, which is the honest
  // signal: nothing can be "verified" without a second model to agree.
  const [pRes, sRes] = await Promise.all([
    runOne(primary),
    secondary ? runOne(secondary) : Promise.resolve({ ok: false, brands: [], costInfo: null, skipped: true }),
  ]);
  ...
  return {
    verified, unverified,
    sources: {
      // `errorKind` and `retriedOn` are what the degradation marker reads: a
      // section labelled "the model returned unparseable JSON" when the real
      // cause was a 401 is worse than no label (R39), so the cause is recorded
      // from the typed error and never inferred from an empty result.
      primary: {
        provider: primary.name, model: primary.model, brands: pRes.brands,
        ...(pRes.error ? { error: pRes.error, errorKind: pRes.errorKind } : {}),
        ...(pRes.retriedOn ? { retriedOn: pRes.retriedOn } : {}),
      },
      secondary: secondary
        ? {
            provider: secondary.name, model: secondary.model, brands: sRes.brands,
            ...(sRes.error ? { error: sRes.error, errorKind: sRes.errorKind } : {}),
            ...(sRes.retriedOn ? { retriedOn: sRes.retriedOn } : {}),
          }
        : { provider: null, model: null, brands: [], skipped: 'single-key mode — no cross-check' },
    },
    costInfo,
  };
}
