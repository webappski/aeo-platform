...
export function renderMarkdown(snapshots, rawResponses = {}, opts = {}) {
  ...
  const sections = [
    ...
    sectionFooter(snapshots, opts.mcMetadata?.identity?.lang),
...
function renderWhiteLabelMarkdown(snapshots, rawResponses, opts) {
  ...
  // three citation sections + sectionCrawlability are each passed
  // { whiteLabel: true }, and the agency / action / outreach / MC sections are
  // not in the list at all. No post-render scrub pass — a regex that strips an
