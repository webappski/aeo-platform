...
export function renderMarkdown(snapshots, rawResponses = {}, opts = {}) {
  ...
  const sections = [
    ...
    sectionFunnelBreakdown(snapshots),        // v0.4 — visibility per intent tag (live since 1.15.1: init now writes tags)
...
function renderWhiteLabelMarkdown(snapshots, rawResponses, opts) {
  ...
  const sections = [
    ...
    sectionScoreRepresentativeness(snapshots, { whiteLabel: true }), // small-N / coverage context — degradation caveat drops the instrument architecture
    sectionKeyMetrics(snapshots),
    sectionEngineRadar(snapshots),
    sectionMatrix(snapshots),                  // query × engine heatmap
    sectionVerbatimQuotes(snapshots, rawResponses),
    sectionSentiment(snapshots, { whiteLabel: true }), // legend keeps the low-confidence flag, drops the two-classifier-model architecture
    sectionFunnelBreakdown(snapshots, { whiteLabel: true }), // table stays; the reading-for-the-client and our config filename do not
    sectionDiff(snapshots),
    sectionRunComparison(snapshots),
