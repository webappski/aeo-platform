...
export function presenceCount(snapshot) {
  // Hits through the one counter (lib/score.js): on a sampled run the brand may
  // have appeared in 1 of 2 trials, and counting the cell's representative
  // label would report that as a whole answer.
  return aggregateScore(snapshot?.results || []).hits;
}
...
export function enginesFullyCovering(snapshot) {
  ...
  for (const r of snapshot?.results || []) {
    const rows = per.get(r.provider) || [];
    rows.push(r);
    per.set(r.provider, rows);
  }
  // "Every answer mentions the brand" = hits equal valid trials, not cells.
  // An engine at 1-of-2 trials on one cell is not fully covering, and an engine
  // whose only measurement errored covers nothing (valid 0 → excluded).
  const full = [...per.values()]
    .map(rows => sliceStats(rows))
    .filter(st => st.valid > 0 && st.hits === st.valid).length;
  return { full, total: per.size };
}
...
export function buildRunMetrics(summary, snapshots) {
  ...
  // Reads the STORED per-run score — the same series the chart plots.
  // Recomputing an old run's components under today's code answers a different
  // question and would put a different number in each place.
  //
  // It is NOT the composite index, and it used to say it was. `score` is
  // `aggregateScore().hits / valid` — the same numerator as the `presence`
  // metric right below, expressed as a share instead of a count. The UVI is a
  // weighted composite of four axes and lands elsewhere: the same client page
  // carried "Visibility index: 14 of 100" beside "Unified Visibility Index: 9",
  // two different quantities under one word. The label now says which one this
  // is; `id` stays `index` because the Mission Control bridge and webappka's
  // admin key off it.
...
export function buildVerdictHeadline(spec) {
  ...
  if (idx.deltaPrev == null) {
    return {
      ...
      segments: [
        { text: `${current}% of answers named or cited you on the first run. `, emphasis: false },
 ...
 *            notAsked: number, total: number, kind: 'lift'|'clean'|'unseen'}}
 */
export function buildLiftOpportunity(snapshot) {
  const results = snapshot?.results || [];
  // `notAsked` is its own bucket, not part of `absent`. Absent means the engine
  // answered and did not name the brand — a real, if unwelcome, measurement.
  // A question a declared subsample never put to the engine measured nothing
  // (AP-RUNMANUAL-DECLARED-SUBSET), and folding it into `absent` would report a
  // deliberately partial leg as 35 fresh failures to be named.
  let named = 0, cited = 0, absent = 0, errored = 0, notAsked = 0;
  for (const r of results) {
    if (r.mention === 'yes') named++;
    else if (r.mention === 'src') cited++;
    else if (r.mention === 'error') errored++;
    else if (r.mention === 'missing') notAsked++;
    else absent++;
  }
  // MEAS-2 — the denominator this aggregate must be read against is the run's
  // VALID trials (lib/score.js), not `results.length`. The two differ the
  // moment a call errors: `total` counts the attempt, `valid` counts the
  // measurement, and the headline score divides by the latter. Both are
  // published so no reader has to guess which one a percentage used.
  const counts = aggregateScore(results);
  ...
  return {
    named, cited, absent, errored, notAsked,
...
export function buildLiftNarrative(lift) {
  ...
  // "N of M answers" — M must be the answers we actually got, not the calls we
  // made. A cell whose provider errored produced no answer, so counting it here
  // shrinks the reported share with a call that measured nothing. `valid` is
  // the run's one denominator (lib/score.js); `total` (= attempts) stays on the
  // object for callers that want the attempt count, and legacy objects without
  // `valid` fall back to it so an older snapshot still renders.
  const total = Number.isFinite(lift?.valid) ? lift.valid
    : (Number.isFinite(lift?.total) ? lift.total : 0);
  if (lift?.kind === 'lift') {
    return {
