...
//   openai      — search-capable model, mini preferred within same generation
//                 (higher limits, lower cost); falls back to flagship if mini
//                 not available in the account's /v1/models response.
...
export const FALLBACK = {
  openai:     { main: 'gpt-5-search-api',      classify: 'gpt-4o-mini' },
  anthropic:  { main: 'claude-sonnet-4-6',  classify: 'claude-haiku-4-5' },
  gemini:     { main: 'gemini-3.5-flash',   classify: 'gemini-3.5-flash' },
  perplexity: { main: 'sonar-reasoning',    classify: 'sonar-reasoning' },
};
...
//     (2) Mini preference — within gen, mini-tier is preferred ("полегче" for higher limits and lower cost).
...
async function fetchOpenAIModels(apiKey, baseURL = 'https://api.openai.com') {
  ...
  const sorted = filtered.slice().sort((a, b) => {
    ...
    const dMini = (isMini(b) ? 1 : 0) - (isMini(a) ? 1 : 0);
...
// NOTE — reused verbatim as the Gemini CLASSIFY fetcher too (see CLASSIFY_FETCHERS
// below). Decision (Alex, 2026-07-07): classify should NOT trail a generation
// behind main here, unlike OpenAI/Anthropic — newest-gen flash is both the
// fastest and smartest tier, and at classify-task volume (short structured
// extraction/sentiment/validation calls) the price gap to a previous-gen flash
// is fractions of a cent — not worth chasing. thinkingBudget/-Level is injected
// identically for main and classify calls (see gemini.js) so this model already
// gets auto-thinking on the classify path too.
...
//
// Decision (Alex, 2026-07-08): classify now matches main (`sonar-reasoning`)
// instead of the cheapest plain `sonar`. Two reasons converged: (1) classify
// should not be the cheapest tier for Perplexity specifically — unlike the
// other three providers, Perplexity has no non-reasoning "mid" tier of its own
// to reach for; (2) classify tasks now want thinking enabled too, and
// Perplexity's only "thinking" knob IS the model name (no request-level
// field exists) — `sonar-reasoning` is the one choice that satisfies both at
// once. Same live-try + blind-fallback pattern as the main fetcher above.
async function fetchPerplexityClassifyModel(apiKey) {
  ...
  // /models endpoint failed or empty — use preference chain blindly.
  debugLog('perplexity-classify', 0, 0, [], 'sonar-reasoning (chain fallback)');
  return ['sonar-reasoning'];
}
...
// gemini deliberately points at the SAME fetcher as main (see the NOTE above
// fetchGeminiModels) — every other provider gets its own dedicated fetcher.
