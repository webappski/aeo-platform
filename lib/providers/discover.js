...
export const FALLBACK = {
  openai:     { main: 'gpt-5-mini',            classify: 'gpt-4o-mini' },
  anthropic:  { main: 'claude-sonnet-4-6',  classify: 'claude-haiku-4-5' },
  gemini:     { main: 'gemini-3.5-flash',   classify: 'gemini-2.5-flash' },
...
// Gemini CLASSIFY fetcher — deliberately a GENERATION BEHIND the main flash.
//
...
// gemini-3.x, which multiplies OUTPUT token volume ~5-10x, and gemini-3.5-flash's
// real rate is $9.00/1M out. gemini-2.5-flash instead uses a dynamic
// thinkingBudget (auto-scaled to the task) at $2.50/1M out — ~9x cheaper on the
// short structured classify/extraction/sentiment/validation calls, at no real
// quality loss. So classify pins to the newest STABLE gemini-2.5 flash; main
// stays newest flash. Returns non-null whenever 2.5-flash is in the catalogue so
// it OVERRIDES a stale classifyModel baked into a user's config; null falls
// through to cfg/FALLBACK ('gemini-2.5-flash').
async function fetchGeminiClassifyModel(apiKey) {
  ...
  const { models } = await res.json();
  // 2.5-flash tier only: excludes lite (no thinking) and pro (pricey), plus the
  // usual unstable variants. gemini.js maps 2.5-non-lite → thinkingBudget:-1.
  const pool = (models || [])
    .filter(m => m.supportedGenerationMethods?.includes('generateContent'))
    .map(m => (m.name || '').replace(/^models\//, ''))
    .filter(id =>
      /^gemini-2\.5/i.test(id) &&
      /flash/i.test(id) &&
      !/-lite\b/i.test(id) &&
      !/\bpro\b/i.test(id) &&
      !/-preview/i.test(id) &&
  ...
  const picked = pool.includes('gemini-2.5-flash')
    ? 'gemini-2.5-flash'
