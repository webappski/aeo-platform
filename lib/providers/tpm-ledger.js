 ...
 *
 * Shrink policy (lower always wins): a 429-observed or header limit can only
 * tighten the budget, never loosen it — protects against transient burst-quota
 * readings. ONE exception: a live `'header'` value from a successful 200
 * response is allowed to overwrite a `'persisted'` value UPWARD. Rationale:
 * if OpenAI just confirmed 500k in the response header, keeping a stale 6k
 * from a file written before the provider raised its limits is wrong and
 * causes unnecessary pacing on every subsequent run.
 *
 ...
 * @param {string} cdKey
 * @param {number} limit
 * @param {string} [source='observed']  'observed' from 429 body | 'header' from 200 response | 'persisted' from ledger file
 */
export function learnTpmLimit(cdKey, limit, source = 'observed') {
  if (!Number.isFinite(limit) || limit <= 0) return;
  const s = getOrCreate(cdKey);
  if (s.learnedTpmLimit == null) {
    // No value yet — always accept.
    s.learnedTpmLimit = limit;
    s.lastLimitSource = source;
    return;
  }
  if (limit < s.learnedTpmLimit) {
    // Lower limit = safer → always accept (shrink).
    s.learnedTpmLimit = limit;
    s.lastLimitSource = source;
    return;
  }
  // Growth path: only allowed when a live 200-header supersedes a stale
  // persisted value. All other growth is rejected (too optimistic).
  if (source === 'header' && s.lastLimitSource === 'persisted') {
    s.learnedTpmLimit = limit;
    s.lastLimitSource = source;
  }
}
...
/**
 * Export the learned per-(provider,model) TPM limits as a plain object, so a
 * caller can persist them between runs (AP-RATELIMIT-UX). The in-window token
 * ENTRIES are deliberately NOT exported — they are stale 60s after the run and
 * meaningless next time. Only the durable, slowly-changing learned ceilings
 * (and where we learned them) survive.
 *
 * Shape: { "openai:gpt-5-search-api": { limit: 30000, source: "observed" }, ... }
 *
 * @returns {Record<string, {limit:number, source:string|null}>}
 */
export function exportLearnedLimits() {
  const out = {};
  for (const [cdKey, s] of state.entries()) {
    if (Number.isFinite(s.learnedTpmLimit) && s.learnedTpmLimit > 0) {
      out[cdKey] = { limit: s.learnedTpmLimit, source: s.lastLimitSource || null };
    }
  }
  return out;
}
...
/**
 * Seed the ledger with learned limits from a previous run. Uses the same
 * shrink-only learnTpmLimit() path, so a more-conservative in-memory value from
 * THIS run is never overwritten by a looser persisted one. Malformed entries
 * are skipped silently — a corrupt ledger file must never break a run (a
 * persisted limit is an optimisation, not a correctness requirement).
 *
 * Floor-check: if the persisted value is LOWER than the current static tier-1
 * table for the same key, the entry is discarded as stale. This covers the
 * case where the static table (or the provider's real limit) was corrected
 * UPWARD — e.g. a tier bump raising a model from 200k→500k — but the persisted
 * file was written before the fix. Without this guard, the scheduler would
 * pace forever on the old low number, since learnTpmLimit is shrink-only and a
 * live 200 header can only rescue it on the first successful call, which
 * happens AFTER the pacing decision. (Note: gpt-5-search-api's OWN honest limit
 * is 6k and the static table now agrees, so a persisted 6k for it is NOT
 * discarded — see rate-limits.js.)
 *
 * @param {Record<string, {limit:number, source?:string}>|null|undefined} obj
 * @returns {number} count of limits actually seeded
 */
export function importLearnedLimits(obj) {
  if (!obj || typeof obj !== 'object') return 0;
  let seeded = 0;
  for (const [cdKey, v] of Object.entries(obj)) {
    if (!cdKey || !v || typeof v !== 'object') continue;
    const limit = Number(v.limit);
    if (!Number.isFinite(limit) || limit <= 0) continue;
    // Floor-check: skip if the persisted value is below the static tier-1
    // table — it was learned under an older (wrong) limit and would cause
    // unnecessary pacing on every run until a live 429/header corrects it.
    // We only apply this check if the value explicitly comes from the persisted file,
    // so tests using `importLearnedLimits` to simulate tight observed limits don't get dropped.
    if (v.source === 'persisted') {
      const [provider, ...modelParts] = cdKey.split(':');
      const tier1 = getTier1Limit(provider, modelParts.join(':'));
      if (tier1?.tpm != null && limit < tier1.tpm) continue;  // stale — discard
    }
    learnTpmLimit(cdKey, limit, 'persisted');
    seeded++;
  }
  return seeded;
}
