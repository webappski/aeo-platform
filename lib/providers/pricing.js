...
const PRICING = [
  ...
  // The 5.6 line dropped size suffixes for names — sol (flagship) / terra
  // (balanced) / luna (cheap). Rates below are the ≤272k-context tier, verified
  // 2026-09-01 against simonw/llm-prices (the same feed scripts/pricing-check.mjs
  // diffs against) and matching developers.openai.com the same day. `luna` is the
  // discovery default (newest generation, cheapest tier) so it MUST carry a row —
  // test/pricing-defaults-covered.test.js enforces that, and an unpriced default
  // would report every run as "cost not tracked".
  { prefix: 'gpt-5.6-sol',        inputPer1M: 4.00,  outputPer1M: 20.00 },
  { prefix: 'gpt-5.6-terra',      inputPer1M: 2.00,  outputPer1M: 12.00 },
  { prefix: 'gpt-5.6-luna',       inputPer1M: 0.20,  outputPer1M: 1.20  },
  { prefix: 'gpt-5.5-pro',        inputPer1M: 30.00, outputPer1M: 180.00 },
  { prefix: 'gpt-5.5',            inputPer1M: 5.00,  outputPer1M: 30.00 },
  ...
  // Sonnet 5 needs its OWN row above the family prefix: it is $2/$10 while
  // Sonnet 4.x remains $3/$15, so the broad `claude-sonnet` row overcharged the
  // default answer model by 50%. Anthropic's own docs, read 2026-09-01, state
  // the launch introductory price "is now the standard price" and that "the
  // previously scheduled increase to $3/$15 … on September 1, 2026 will not
  // occur" — i.e. the family row was not merely stale, it encoded a rise that
  // was cancelled. The 4.x row below stays correct for the models it covers.
  { prefix: 'claude-sonnet-5',    inputPer1M: 2.00,  outputPer1M: 10.00 },
  ...
  // the 3.6-flash and 3.5-flash-lite rows re-verified there 2026-07-31).
  //
  // gemini-3.6-flash is what the API SERVES for a config that pins 3.5-flash
  // (model drift, lib/providers/model-drift.js). Without its own row the served
  // id fell through to null → every main Gemini call in a run recorded as
  // "cost not tracked", so an ~$1.60 run reported $0. Pricing must cover SERVED
  // ids, not only configured defaults (which is all pricing-defaults-covered
  // guards) — when a provider starts serving a new id, add its row here.
  //
  // gemini-3.5-flash-lite likewise needed its own row: without it the id
  // prefix-matched the 3.5-flash row and billed at $1.50/$9.00 instead of
  // $0.30/$2.50 — the exact silent-overcharge this table's ordering rule exists
  // to prevent.
  ...
  // Order matters: '-flash-lite' MUST precede '-flash' so the longer prefix wins.
  // gemini-3.8-flash is the newest flash and therefore what discovery now picks
  // (and what DEFAULT_CONFIG/FALLBACK seed, bumped 2026-09-08 — see
  // lib/config.js). Added its row up front this time, precisely to not repeat
  // the 3.7 gap below.
  //
  // gemini-3.7-flash was the newest flash and therefore what discovery picked
  // (and what DEFAULT_CONFIG/FALLBACK seeded). It shipped without a row, so the
  // 2026-09-01 run printed «cost not tracked for: gemini-3.7-flash» and every
  // main Gemini cell contributed $0 to the session total.
  //
  // 3.6, 3.7 and 3.8 share one introductory schedule, verified 2026-09-08 on
  // ai.google.dev/gemini-api/docs/pricing (3.8 row cross-checked against 3.7's
  // own page the same day — identical numbers on both):
  // "$0.75 through December 31, 2026. $1.50 starting January 1, 2027" (output
  // $3.75 → $7.50). The 3.6 row once carried the 2027 number — the post-promo
  // rate read as if it were today's — so every 3.6 cell was billed at ~2×
  // reality; corrected 2026-09-01. Revisit all three rows on 2027-01-01.
  { prefix: 'gemini-3.8-flash',       inputPer1M: 0.75,  outputPer1M: 3.75  },
  { prefix: 'gemini-3.7-flash',       inputPer1M: 0.75,  outputPer1M: 3.75  },
  { prefix: 'gemini-3.6-flash',       inputPer1M: 0.75,  outputPer1M: 3.75  },
  { prefix: 'gemini-3.5-flash-lite',  inputPer1M: 0.30,  outputPer1M: 2.50  },
...
function findPricingRow(model) {
  const row = PRICING.find(r => model.startsWith(r.prefix)) || null;
  // A "-lite" model is a separate, much cheaper SKU. If the only row it matched
  // is its non-lite sibling — because its own row is missing, or is ordered after
  // the shorter prefix — charging it the sibling's rate overstates cost 3-5x.
  // Refuse the match and return null: an honest "cost not tracked" beats a
  // confidently wrong number, the same call this table makes by carrying no
  // provider-wide catch-all. Ordering alone can't cover a lite variant that
  // ships before we notice it (gemini-3.6-flash had no lite sibling at the time
  // its row was added); this makes the fallthrough impossible rather than
  // dependent on someone keeping the list ahead of the vendor.
  if (row && model.includes('-lite') && !row.prefix.includes('-lite')) return null;
  return row;
}
