...
const PRICING = [
  ...
  // gemini-3.7-flash is the newest flash and therefore what discovery now picks
  // (and what DEFAULT_CONFIG/FALLBACK seed). It shipped without a row, so the
  ...
  //
  // 3.6 and 3.7 share one introductory schedule, re-verified 2026-09-01 on
  // ai.google.dev/gemini-api/docs/pricing AND in the simonw/llm-prices feed:
  // "$0.75 through December 31, 2026. $1.50 starting January 1, 2027" (output
  // $3.75 → $7.50). The 3.6 row carried the 2027 number — the post-promo rate
  // read as if it were today's — so every 3.6 cell was billed at ~2× reality.
  // Corrected to the rate in force now; revisit both rows on 2027-01-01.
