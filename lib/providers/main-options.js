...
//   - openai:    main injects NOTHING (`{}`) → the model uses its own adaptive
//                default effort (~medium for gpt-5.x). OpenAI has no `'auto'`
//                effort value (only none/minimal/low/medium/high/xhigh), so
//                "let the model decide" = omit the field. This replaced a forced
//                `reasoning_effort: 'high'` that ballooned reasoning-output
//                tokens on the mini (decision Alex, 2026-07-14 — main answer just
//                needs to mirror a typical ChatGPT reply, not deep reasoning; a
//                one-word `'low'`/`'minimal'` here would cut further if wanted).
//                classify stays `'low'` but the classify model (gpt-4o-mini) is
//                non-reasoning so openai.js's gate drops it anyway (no-op).
...
export const MAIN_OPTIONS_BY_PROVIDER = {
  openai:     {},  // omit reasoning_effort → model-default adaptive effort (see note above)
