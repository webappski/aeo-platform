 ...
 * This is why the classify tier is pinned to gemini-2.5-flash (dynamic budget)
 * rather than a gemini-3.x flash (forced high). Exported so the tier→thinking
 * mapping is unit-testable without a live API call.
 * @param {string} model
...
export async function callGemini(query, apiKey, model, options = {}) {
  ...
  //   (The tier choice happens upstream: classify is pinned to gemini-2.5-flash
  //   so it lands on the dynamic thinkingBudget path, not forced-high.)
