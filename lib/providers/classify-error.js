 ...
 * @property {'billing'|'auth'|'rate-limit'|'model-deprecated'|'offline'|'network'|'server-error'|'site-fetch'|'bot-protection'|'config'|'filesystem'|'other'} category
...
export function classifyProviderError(err) {
  ...
  // Offline refusal (`report --offline`). FIRST, and as its own category: we
  // chose not to ask, which is not the same fact as asking and failing, and a
  // report must never present the first as the second. It also must not reach
  // the quick-retry ladder — `network` and `server-error` are retried on the
  // theory that the next attempt might connect, and here it provably cannot.
  if (code === 'EOFFLINE' || err?.name === 'OfflineError') {
    return {
      retryable: false,
      category: 'offline',
      reason: 'offline mode — no request was made',
      fixHint: 'drop --offline to let this run fetch what is missing from the cache',
    };
  }
