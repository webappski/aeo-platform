...
export const TIER_1_LIMITS = {
  openai: {
    // The legacy `-search-api`/`-search-preview` SKUs have their OWN small
    // dedicated bucket — NOT the general gpt-5 500k cap. gpt-5-search-api was
    // measured at 6k TPM on tier 1 via a live x-ratelimit-limit-tokens header
    // (2026-07-14). We no longer route web search through this SKU (it's the
    // Responses `web_search` tool on a general mini now — see openai.js), but
    // the honest number stays here so an accidental override paces correctly
    // AND so importLearnedLimits' floor-check doesn't discard a learned 6k as
    // "stale" against a fictional 500k.
