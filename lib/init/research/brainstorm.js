 ...
 *   - Guard 1: expand only AMBIGUOUS industry acronyms (see acronyms.js); keep
 *     universally-understood ones (AI, API, SaaS…) as-is.
...
import { AMBIGUOUS_ACRONYMS, UNIVERSAL_ACRONYMS } from './acronyms.js';
...
export function buildBrainstormPrompt({ brand, domain, site, categoryDescription, audienceTags = [], geoTags = [], avoidFeedback = [] }) {
  ...
  // Acronym rule inputs — expand ONLY ambiguous acronyms, keep universal ones.
  // Both lists come from acronyms.js so the generator and the filter agree (AI
  // was ballooning into "Artificial Intelligence" under the old "expand EVERY
  // acronym" rule, which the filter never even flagged).
  const acronymExpandList = AMBIGUOUS_ACRONYMS.map(a => `"${a.abbr}" → "${a.expansion}"`).join('; ');
  const acronymKeepList = UNIVERSAL_ACRONYMS.join(', ');
...
NON-NEGOTIABLE RULES (violations are rejected):
  ...
  A. **Acronym expansion (disambiguation only).** Spell out ONLY these genuinely
     ambiguous industry acronyms: ${acronymExpandList}. This applies EVEN IF the
     CATEGORY_DESCRIPTION above uses the abbreviation.
     Do NOT expand universally-understood acronyms — keep ${acronymKeepList} exactly
     as written. Real users search "best AI voice tools", not "best Artificial
     Intelligence voice tools".
...
export function parseBrainstormResponse(text) {
  if (!text || typeof text !== 'string') throw new LlmParseError('Empty brainstorm response');
  ...
  throw new LlmParseError('Could not parse JSON from brainstorm response');
}
...
export function validateBrainstormShape(parsed) {
  if (!parsed || typeof parsed !== 'object') {
    throw new LlmParseError('Brainstorm output is not a JSON object');
  }
  ...
  if (totalAcross < TARGET_TOTAL_MIN) {
    throw new LlmParseError(`Brainstorm produced only ${totalAcross} commercial queries — need at least ${TARGET_TOTAL_MIN}`);
  }
...
export async function runBrainstorm({
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      // limit errors are already retried inside providerCall (withRetry); this is
      // the fatal phase, so a real provider error should surface fast to the init
      // fallback loop, not after another full round.
