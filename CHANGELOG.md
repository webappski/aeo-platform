...
Минорная. Добавился один флаг (`report --offline`) и рядом с ним два новых отказа с кодом `2` (вместе с `--refresh-cache` и на любой команде кроме `report`) — остальные команды и коды возврата прежние. **Что обязан учесть тот, кто читает конфигурацию программно: `init` теперь сохраняет вопрос как `{"q": "…", "tag": "…"}` вместо простой строки.** Обе формы читаются как и раньше, конфигурация из строк работает без изменений и ничего не теряет, но свежесозданная несёт объект. Числа и их источники не менялись. Меняется ТЕКСТ, который видит клиент: два блока в `report --white-label`, имя движка в одной оговорке, подпись заголовочной метрики во всех трёх режимах и разбивка по вопросам, которая больше не называет себя воронкой.

### Added

- **`report --offline` — отрисовать отчёт из того, что уже на диске: ни одного запроса и ни одной записи.** Перерисовка прошлой даты считалась бесплатной и таковой не была. Замерено на копии клиентской даты со счётчиком на каждом исходящем: `report --for-date` сделал **45 исходящих попыток** на холодном кэше при ВСЕХ четырёх выставленных `--no-*` — у трёх добытчиков (`citationClassification`, `llmActions`, `crawlability`) флага не было вовсе, и один из них платный. Отдельно и независимо: на ТЁПЛОМ кэше, где запросов ноль, исходный `_summary.json` всё равно переписывался — `regionContext` и `responseFreshness` это чистые деривации, которые сохраняются при каждом отчёте. То есть перерисовка и тратила деньги, и правила запись, уже отданную клиенту.

  Гейт стоит в ДВУХ местах, по одному на свойство: отказ от сети — в `lib/util/fetch-with-timeout.js` (единственный модуль в `lib/`, который зовёт `fetch`), отказ от записи — в `persistSnapshot`. Не у полей: проверка «по полю» и есть та форма, которая породила баг — флаг писали для каждого добытчика отдельно, потом добавили ещё три, и ни один флага не получил. Добытчик, написанный завтра, получает запрет даром и не может забыть подписаться. Отсутствующее в кэше помечается как «не добыто», а не угадывается; отказ типизирован (`EOFFLINE`), чтобы «мы решили не спрашивать» нигде не выдавалось за «спросили и не вышло» и не попадало в лестницу повторов.

  Два случая, где флаг отказывает вместо тихого no-op (кодом 2, с названной альтернативой): вместе с `--refresh-cache` — потому что чистить кэш, чтобы добытчики перезапустились, в режиме без добытчиков значит потерять секции, которые отчёт ещё мог показать; и на любой команде кроме `report` — таблица опций общая, поэтому `run --offline` разбирался бы успешно и не делал ничего, а у `run` для этого есть свой бесплатный путь, `--replay`.

### Fixed

- **`init` считал интент каждого вопроса и выбрасывал результат, из-за чего секция разбивки не рендерилась НИ РАЗУ.** `lib/init/research/classify-intent.js` относит каждый отобранный вопрос к одному из пяти классов (`comparison` / `problem` / `commercial` / `informational` / `vertical`) локальными регулярками поверх разметки brainstorm-LLM — собственного вызова у него нет. Вердикт жил только в памяти процесса и в конфиг не попадал, поэтому ни один прогон не нёс `tag`, фильтр секции всегда опустошался, и четыре минорные версии кода ниже этого фильтра не исполнялись ни разу. Теперь оба писателя конфигурации (первичный `init` и `init --queries-only` в обоих режимах) штампуют уже посчитанный класс общим `stampQueryAxes`. **Ни нового запроса, ни нового вопроса при установке, ни одного платного вызова** — классификация уже произошла, её перестали терять.

  Ключ — ТЕКСТ вопроса, не позиция: валидатор-recovery подменяет заблокированный вопрос кандидатом из пула по тексту, после чего позиционный массив интентов описывает корзину, которой больше нет. Записываются и отобранные, и пул — иначе подменённый вопрос приезжает без тега и корзина выходит наполовину размеченной. Вопрос, который классификатор не взялся отнести, **остаётся простой строкой**: фолбэк отбора, превращающий неизвестное в «commercial», существует для ранжирования кандидатов и, попав в конфиг, публиковал бы догадку как измеренный класс. Конфиг из простых строк байт-в-байт совпадает с тем, что писали прежние версии, и работает без изменений.

- **Секция называла воронкой то, чего мы не меряем.** Мы считаем интент вопроса, а дефолтная подпись шла дальше и вычитывала из него историю «high ToFu, zero BoFu». Мапинг интентов на модель воронки — наше суждение о чужом рынке, и напечатанное рядом с настоящими процентами оно выдаёт суждение за измерение. Заголовок, подпись карточки, обе ветки прозы и строка прогона в терминале теперь говорят то, что считают. Три из шести мест нашёл тест, а не чтение исходника: комментарии лежат в `styles.css`, который встраивается в single-file HTML и уезжает клиенту.


- **Классификатор интентов не запускался ни разу на сайте, который объявляет язык с регионом.** `INTENT_PATTERNS` ключуется базовым субтегом (`pl`), а в него приходит СЫРОЙ атрибут `<html lang>` измеряемого сайта — а страницы пишут `pl-PL`, `de-DE`, `en-US`, иногда `PL` или `en_GB`. Каждый такой вид промахивался, `reconcileIntent` уходил в ветку `lang-unsupported` и оставлял тег brainstorm — а тот всегда `commercial`. Замерено на 14 корзинах с диска: под любой региональной формой **284 вопроса из 284 давали null**. Нормализация (`normalizeLang`) применяется в ОБОИХ входах: у `reconcileIntent` своя проверка поддержки языка, и правка только в `classifyIntent` оставила бы верную классификацию, которую тут же выбрасывают. В `fetch-site.js` источник НЕ трогали: оттуда тот же атрибут идёт в промпт brainstorm, а правка промпта — отдельная работа со своим замером.

- **Польский и немецкий знали превосходную степень в одной-двух формах из парадигмы.** Покупатель пишет ту, что согласуется с существительным: «najlepsza agencja» — самая частая форма в реальных польских корзинах — не ловилась ничем, как и `bestes` / `besten` / `bestem` в немецком, притом что `besten` стоит в самой ходовой немецкой покупательской фразе. Добавлены ценовые формы (`ile kosztuje`, `cennik`, `was kostet`). Перечислением, а не основой, где основа опасна: `best\p{L}*` поймала бы «bestellen», `cen\p{L}*` — «centrum». Замер до/после на тех же 14 корзинах: по базовому `pl` 15 вопросов перешли из null в `commercial`, ещё 6 — из `vertical` в `commercial` (приоритет `commercial` выше `vertical` был и раньше; правка лишь применила существующее правило последовательно, как это давно работает в английском).

- **Русского набора шаблонов не было вовсе**, поэтому для русского сайта классификатор не запускался никогда. Написан сразу на Unicode-границах: `\b` в JavaScript ASCII-only даже под флагом `u`, так что `/\bлучший\b/` не совпадает ни с чем — шаблон, скопированный в прежнем ASCII-стиле, был бы мёртв и выглядел бы как «русские вопросы просто не классифицируются». На диске 32 русских вопроса (включая нашу собственную корзину webappski), 22 из них теперь несут класс; оставшиеся 10 не несут ни покупательского, ни сравнительного маркера — это верный ответ, а не пропуск.

- **Разбивка из одной корзины больше не рендерится.** Одна строка на 100% выглядит как измерение и им не является: «все ваши вопросы коммерческие» — факт о корзине, а не о видимости. Состояние достижимое, а не теоретическое: пока язык не читался, все вопросы получали `commercial` от brainstorm. Скрывается одинаково в markdown, white-label и HTML — по той же причине, по какой секция скрыта, когда тегов нет совсем.


- **Корзина собрана по РЫНКАМ, а язык у прогона был один — сайта.** Обнаружено при замере предыдущего пункта: `webappka` несёт 30 польских, 15 английских и 5 русских вопросов в ОДНОЙ корзине, а сайт объявляет `lang="en"` — значит три вопроса из четырёх читались не теми шаблонами и падали в запасной тег. Язык сайта отвечает на вопрос «для кого этот отчёт», а не «на каком языке это предложение». Теперь язык определяется для КАЖДОГО вопроса (`detectQueryLang`), язык сайта — только запасной путь. Детерминированно, без зависимостей и без сети: буква, существующая ровно в одном из поддерживаемых языков (кириллица → ru, `ąćęłńóśźż` → pl, `äöüß` → de), иначе служебные слова, иначе — откат. Неоднозначное (польское и немецкое слово в одной строке) откатывается, а не угадывается; слова, общие с английским (`top`, `ranking`, `firm`), в определитель намеренно не включены — ложное срабатывание отправило бы английский вопрос к чужому набору шаблонов.

  Замер до/после на тех же 14 корзинах методом пайплайна (`lang="en"`, как объявляют наши сайты): распознано шаблонами **60 → 210** вопросов из 284; итоговые теги были 272 `commercial` / 8 `vertical` / 3 `informational` / 1 `comparison`, стали **232 / 44 / 7 / 1**. То есть разбивка у нас самих перестала быть вырожденной и секция снова что-то значит. **Контроль: ни одна одноязычная корзина не сдвинулась ни на строку** — ни по классу, ни по тегу (проверено по всем 14; двигаются только 4 смешанные). Откат равен прежнему поведению, поэтому правка может только добавить классификацию и не может отнять её у вопроса, который уже читался верно.


- **`--white-label` рассказывал клиенту, как устроен наш инструмент.** Оговорка «что не удалось классифицировать» и легенда секции тональности рендерились одинаково во всех режимах, поэтому отчёт, чья вся легенда — «самодельный локальный статистический инструмент», сообщал, что каждую ячейку перекрёстно проверяют **две грейдинг-модели**, что одна из них вернула **невалидный JSON** и что упавшее место **переспрашивают**. Имя вендора не называлось никогда (это было решено сознательно), а вот архитектура — называлась. Молча убрать оговорку было бы хуже утечки: это обмен тельца на сокрытие того, что часть ответов — более слабое свидетельство. Поэтому оговорка остаётся, уходит только машинерия: в white-label те же факты звучат как «N ответов классифицированы с пониженной надёжностью» и «оценка тональности сохраняется, но помечается как низкоуверенная». Одна формулировка на обе поверхности (`lib/report/section-degradation.js`, `lib/report/sections.js#sectionSentiment`) — не две копии, которым есть куда разойтись.

- **Оговорка о заявленной подвыборке печатала сырой id провайдера.** В markdown стояло «anthropic answered 15 of 50 questions» на странице, которая везде остальной говорит Claude — два имени одного движка читаются как два движка. Теперь через тот же `providerLabel`, которым пользуется весь отчёт, а не через второй словарь.

- **На одной странице два разных числа назывались индексом видимости.** Шапка отчёта печатала «**14 of 100 on the first run**» и «**Visibility index:** 14 of 100», а блок ниже — «Unified Visibility Index — 9 / 100». 14 это `aggregateScore().hits / valid`, доля ответов, назвавших или процитировавших бренд: тот же числитель, что у строки «Answers naming or citing you» прямо под ней. 9 — взвешенный композит четырёх осей. Ни одно из чисел не было неверным, неверной была подпись, и неверная стояла первой. Доля теперь называется долей и печатается процентом (`Share of answers naming or citing you`, `14%`); словом «индекс» на странице зовётся только UVI. Числа и их источник не изменились: `id` метрики остаётся `index`, ряд `trend` прежний, сопоставимость с уже отданными документами не нарушена.

- **Пояснение к расхождению этих двух чисел называло причину, которой нет.** Когда доля и композит расходились, отчёт печатал: «разрыв означает, что правила скоринга изменились с момента замера». Наблюдать такое невозможно: `summary.score` заполняется долей у всех трёх писателей (`bin:3989`, `sections.js:3538`, `comparison-payload.js:118`), то есть эти числа никогда не были одной величиной в двух редакциях правил и расходились бы, даже если бы ни одно правило не менялось. Теперь сказано то, что есть: это два разных измерения одного прогона, и объяснено, почему ось с малой выборкой двигает одно и не двигает другое.

### Tests

- **Новый тест классификатора** (`test/classify-intent.test.js`) — модуль не был покрыт ничем. Ядро файла — таблица форм: падение называет КОНКРЕТНУЮ форму, которая отвалилась, а не одно предложение на язык. Плюс ось языкового тега (`pl`, `pl-PL`, `PL`, `pl_PL`, ` pl-pl `), утверждение именно на `reconcileIntent` (иначе половинчатая правка зеленеет), проверка на точность границ («kostenlos» не цена, «centrum» не «cena», «ценность» не «цена») и проверка, что каждый русский шаблон вообще способен совпасть — гарантия против ловушки `\b`.

- **Новый E2E на `diff --since <date>`** (`test/e2e/diff-since.test.js`, FC-03 из бэклога — первый в очереди четыре цикла подряд). `diff A B` получает оба конца, `--since` выбирает второй САМ, и неверный выбор не падает, а показывает настоящую разницу между не той парой — в команде, вся работа которой отчитаться об изменении. Фикстура обязана различать: на двух прогонах «последний» и «следующий за A» — одна и та же папка, поэтому staging из трёх, где средний совпадает по score с первым (40 / 40 / 50). Правильный выбор даёт +10pp, ошибочный — 0pp, и отдельный тест показывает это противопоставление, а не предполагает. Мутация «сдвиг на один» проверена КРАСНОЙ.

- **Новый двусторонний E2E на теги** (`test/e2e/report-intent-tags.test.js`) — размеченная корзина сегментирует отрисованный отчёт; корзина из простых строк по-прежнему проходит прогон и просто не несёт секции. Односторонняя проверка зеленела бы и в случае, когда сломана вторая форма. Запрет на слово «воронка» утверждается ТОЛЬКО после контроля, что секция вообще отрисовалась: против пустого рендера он проходит вхолостую — ровно то состояние, в котором находился каждый прежний прогон. Пять мутаций проверены красными. Контракт записи запинен отдельно на настоящем модуле (`test/queries-normalize.test.js`): его единственный живой производитель — research-пайплайн, а он тратит деньги.

- **Посеянный `validationCache` в E2E-хелпере ключевался самой записью корзины, а не её текстом.** Для корзины из объектов в поле `query` ложился объект, валидатор сравнивает его с текстом кандидата, все попадания промахивались — и прогон уходил в ЖИВОЙ валидатор. `spawnCli` подставляет фальшивый ключ только если своего в окружении нет, поэтому на машине с настоящим `OPENAI_API_KEY` такой промах тратит деньги: замерено, ~$0.0022. Бесплатность сюиты держалась на попадании в кэш, а не на заглушке сети. Хелпер читает текст через `queryText`, а новый E2E идёт под `offlineFetchEnv` — сеть заглушена, платный вызов физически невозможен.


- **Три теста на утечку были зелёными вхолостую.** Их фикстуры не несли ни одного поля, которое включает условные блоки отчёта (`extractionSources`, `degraded`, `errorKind`, `declaredSubsets`, ячеечный `sentiment`, `modelDrift`/`resolvedModel`, `costTracked:false`), поэтому эти блоки в рендере попросту отсутствовали, а денилисты проходили мимо, не прочитав ни слова. **Запрет проверяет не то, что в нём перечислено, а то, что фикстура способна произвести.** Общее семя (`test/e2e/_helpers.js#conditionalSurfaceCells`) добавляет по ячейке на условие поверх уже настроенных фикстур; `degraded` не вписан руками, а выведен тем же `degradationsFor()`, которым его пишет настоящий прогон. Обе утечки выше нашлись именно этим семенем. Плюс закрыта поверхность, о содержимом которой не было ни одного утверждения: в `--public` мост Mission Control сохраняется (R8) и встраивает свой JSON прямо в отдаваемый HTML, так что строгий allow-list `perCell` — несущий гейт.

- **Новый E2E на `--offline`** (`test/e2e/report-offline.test.js`) — три режима × ноль исходящих, плюс хеш сводки до/после, плюс отказ на противоречивой паре флагов. Запросы не «считаются по коду», а ЗАМЕРЯЮТСЯ: подпроцесс идёт под преднагрузкой, которая подменяет `fetch`, `http.request` и `https.request` записывающими отказами, — заглушка только на `fetch` не увидела бы модуль, дёрнувший `node:https` напрямую. Поля намеренно НЕ перечисляются: перечень проверял бы ровно то, что уже сломалось. И есть контроль — тот же холодный прогон БЕЗ флага обязан и ходить в сеть, и переписывать сводку, иначе все четыре проверки зеленели бы на фикстуре, которой нечего добывать. Мутации проверены: снятие сетевого гейта роняет три теста на ноль запросов, снятие гейта записи — только тест на неизменность.

- **Новый E2E на то, что доля публикуется долей** (`test/e2e/report-index-word-one-number.test.js`) — обе поверхности, все три режима. Фикстура намеренно построена так, чтобы доля и композит расходились (21 ячейка, тональность на двух, ранг на одном — гейт малой выборки срабатывает): на прогоне, где они совпадают, любая такая проверка зеленела бы вхолостую, поэтому расхождение утверждается ПЕРВЫМ. Проверка двусторонняя — доля не должна печататься как «N of 100» и обязана печататься как «N%»: односторонний запрет одинаково рад и исправлению, и исчезновению цифры. И намеренно НЕ проверяется «доля не стоит рядом со словом index»: само исправление содержит фразу, которая называет оба числа, чтобы их ПРОТИВОПОСТАВИТЬ, и правило близости объявило бы дефектом именно её.

- **HTML-отчёт показывает разбивку по интентам вопросов**, которую собирал и выбрасывал с тех пор, как секция была написана. Для всех уже сохранённых прогонов это no-op: секция не рендерится, если ни одна ячейка не несёт `tag`, а таких прогонов на диске нет ни одного. Зато делает достижимой дремавшую утечку white-label, поэтому она закрыта тем же коммитом: подпись секции читала таблицу ЗА клиента и называла наш файл конфигурации по имени — совет и отпечаток инструмента в документе, который идёт под чужой легендой и несёт только статистику. В white-label оба уходят, таблица остаётся в обоих режимах: отнять цифры значит сделать документ сломанным, а не сдержанным. Утечка жила в markdown-рендерере white-label и просто ни разу не срабатывала, потому что ни один прогон не мог включить секцию.

### Docs

- **`AGENTS.md` доведён до самодостаточности: агент больше не обязан открывать README.** README — 105 КБ (~26 К токенов) публичной витрины, которую цитируют движки; агент, зашедший в него за поведением кода, платил 26 К токенов за текст, написанный для покупателей. Теперь точка входа — `AGENTS.md` (~13 КБ, ~3,2 К токенов) плюс `--help`: вместе они покрывают установку (`npx` / глобальная), конфигурацию (ключи — только из окружения, `.aeo-tracker.json` пишет `init`), прогон и разработку. Перечня команд в `AGENTS.md` намеренно нет — `--help` остаётся единственным авторитетом, чтобы списку было негде протухнуть. README не сжимался: это цитируемая поверхность, резать её — платить видимостью за удобство чтения.
- **Названо, чем именно запинён README** — четыре E2E-файла и релизный скрипт, плюс красные линии (три `##`-строки заморожены посимвольно; проза FAQ и FAQPage правятся только парой; `docs/` не добавлять в `files[]`). Переформатирование блока Schema.org ломает не тест, а выпуск, и об этом теперь сказано до правки, а не после.
- **Два урока этой партии — в раздел про фикстуры:** запрет проверяет не то, что в нём перечислено, а то, что фикстура способна произвести; форму поля фикстуры сверять с тем, кто пишет его в настоящем прогоне (лучше — выводить тем же кодом). Второй урок записан по следам собственной ошибки: непроверенная форма `declaredSubsets` дала фантомную утечку.

## [1.15.0] — 2026-09-20

Минорная. Команды и коды возврата прежние, добавился один флаг (`run-manual --declared-subset`) и одно выключенное по умолчанию поле. **Что обязан учесть тот, кто читает вывод программно: сводный индекс видимости считается по новой формуле и помечен `scores.uviVersion: 2` — сохранённые числа v1 с ним несопоставимы**, их нужно пересчитать из сырых ответов, а не сравнивать с прежним значением. `scoreDelta` получил вторую причину обнуления (`engines-changed`) рядом с `basket-changed` из 1.14.0.

### Changed

- **An axis measured on one or two cells carried the same weight as an axis measured on twenty.** The unified visibility index weighted every measured axis alike, however little stood behind it. On a real 21-cell run the brand appeared in 3 answers of 21, and the index published **44 of 100** — because a sentiment averaged over TWO cells and a rank taken from ONE, both echoes of the same brand-probe question, carried **45% of the weight**. Under v2 a small-sampled axis is **reported but not counted** (it keeps its value and gains `counted: false`, `lowConfidence`, `excludedReason`, and both report surfaces print «n=2 · low confidence — not counted»), presence and citation re-normalise over the remaining weight, and that run scores **9**. The guard may narrow the composite, never empty it: if every measured axis is small-sampled it stands down rather than publish a 0 nobody measured. The gate lives in `lib/report/visibility-index.js` and nowhere else — `lib/report/mc-metadata.js#scores()` delegates to it rather than keeping a second copy of the formula, which is a drift that had to be closed once already. **A baseline recorded under v1 is not comparable to a v2 number** — re-score the earlier run from its raw responses instead of subtracting the two.

- **New: `run-manual --declared-subset=<name>` — a leg that is partial ON PURPOSE can now be imported instead of refused.** `--from-dir` rejected a directory missing any `q<N>.txt`, and that refusal is right for an accidental gap; what it could not express was a deliberate subsample (ours runs 15 of 50 questions on one engine for continuity, because the full leg costs ~92k tokens a question). The refusal stays the default; partiality becomes declarable. Uncovered questions are written as real cells with `mention: 'missing'` — never asked, therefore not a hit, not a miss, and **in no denominator** — and the summary carries a per-provider stamp naming the subsample, its coverage, the question numbers, and a selection-bias sentence that is not optional (a subsample drawn from questions where the brand was already visible cannot publish a visibility rate for the basket). Without the flag, behaviour and summary shape are unchanged. This closed a class, not one call site: **four surfaces read "not an error" as "measured"** — the UVI presence and citation denominators, the representativeness sample, the displacement-gap filter and the lift buckets — and would have read 35 unasked questions as real cells. All four now ask `lib/score.js`, which has held one definition of "this cell produced a measurement" all along.

- **A changed ENGINE SET withholds the headline delta, the same way a changed basket does (1.14.0).** Dropping or adding an engine between two runs changes what the number measures, but `scoreDelta` reported the difference anyway, with no reason attached. The engine axis now lives in `lib/run-axes.js` and is read by **both** `lib/diff.js` and the report header — the header did its own `latest.score - prev.score` and never went through `diff.js` at all, which was the second half of the defect. On the real pair 2026-08-31 (three engines, 150 cells, 13%) → 2026-09-16 (two engines, 100 cells, 11%, the Claude leg not run) the tool published `scoreDelta −2` with an empty reason, while over the 100 cells the two runs share the score was **11 against 11 — no movement at all**. It is now `null` with `scoreDeltaReason: 'engines-changed'`, the engines that entered or left are named, and the report header states «3 engines measured then, 2 now» instead of quietly dropping its arrow. An engine whose every cell errored or was never asked measured nothing and does not count as coverage. `scoreDeltaReason` still yields `'basket-changed'` first when both axes moved, so consumers that only knew that string keep working; the full list arrives in `scoreDeltaReasons`.

- **New, and OFF by default: `mentionRole` — being named as a supplier a buyer could hire is not the same as being named as the author of a list about rivals.** A run counted both as `mention: 'yes'`. On the 2026-09-16 run three cells were formally right and substantively backwards — in one, the answer lists four hireable suppliers and then names us as the author of a ranking *about them*; in two more, our article sits among the sources *about* the competitor being recommended. A headline "6 → 8" built on those is not a gain in recommendations. `mentionRole` is a **new field beside `mention`, never a new value inside it** (consumers branching on `'yes' | 'src' | 'no'` keep working), filled by its own cheap classify-tier call in the existing parallel batch, fired only on cells that named or cited the brand — no existing prompt was touched. Enable with `classifyMentionRole: true` in `.aeo-tracker.json`; the gate is a strict `=== true`, so a project that wrote `"yes"` does not start spending, and the run announces the line item before the first call. Cost when enabled: **$0.0045–0.015 per run** at 11–25 mentioned cells, against $1.88 for the whole run it was measured on. With it off, every published number and the Mission Control payload are byte-identical to before. **Its accuracy on live answers is not proven** — the tests exercise the real prompt, parser and merge against fake providers, so what is demonstrated is the wiring and the shape of the output, not the verdicts.

- **A classify-tier answer that came back as garbage is now re-asked once, instead of costing the cell its cross-check.** The competitor and sentiment sections ask two models and compare them. When one returned something that was not JSON, that seat was simply lost: the section degraded to single-model for that cell, every competitor name fell into the `unverified` tier, and the run had paid for an answer it threw away. A malformed body is the one provider failure a second ask reliably fixes. **Only `LlmParseError` is re-asked** — provider / network / rate-limit failures are already retried exhaustively inside the transport, and re-running the classification on top of that multiplies an exhausted chain. One extra attempt, never a loop. The re-ask goes to a **different vendor when one is available**: `buildExtractionProviders` builds every research-capable provider and used to discard the third, which is now the preferred answerer; with the usual two keys there is no third vendor and the re-ask goes back to the same model, which is the same pattern `runBrainstorm` and `runSimulation` have always used. **Independence is not faked** — the re-ask replaces ONE seat, so the cross-check is still seat-against-seat and no path turns one model's two samples into "both models agreed". Cost: at most one extra classify-tier call per failed cell, ~$0.0008 at the cheap tier, and only on a parse failure. `prose-rank` and `mentionRole` deliberately do **not** get the re-ask yet — both are days old and one is off by default.

- **A report section that lost a classification now says so, on both surfaces.** When a classify call failed, the competitor list quietly got shorter and a cell's sentiment quietly became single-model or vanished. Both are the correct behaviour — the alternative is inventing a verdict — but nothing anywhere said a degradation had happened, so "only two competitors this week" read as a finding about the market rather than as a call that came back broken. Each cell now records **why** (`degraded: [{section, provider, model, kind, recovered}]` in `_summary.json`, omitted entirely on a clean cell), the run rolls those up into one counted line per cause, and the same sentences are printed by the markdown report and by the HTML page a client is shown — one builder, `lib/report/section-degradation.js`, because a caveat two surfaces word differently is a caveat a reader can shop around. **The cause is read from the typed error, never inferred from an empty result**: a marker that blames unparseable JSON for what was really an expired key sends the reader to fix the wrong thing, so a failure that arrived without a kind is reported as an unnamed failure rather than guessed into one. A recovered re-ask is listed too — nothing was lost, but the extra call is visible. The note **renders in `--public` and `--white-label` as well** — a client reading a short competitor list is exactly the reader who needs it — and it **never names the grading model**: those vendor names are the same ones this report uses for answer engines, so "Gemini returned unparseable JSON" beside a competitor list reads as "your Gemini measurement is broken", a second wrong conclusion inside the copy written to prevent the first. The vendor stays in `degraded[].provider` for whoever debugs the run, and is dropped from the Mission Control payload with the rest of the extractor plumbing.

### Added

- **`AGENTS.md` at the repository root** — the [agents.md](https://agents.md) format, which coding agents (Codex, Jules, Cursor, Copilot, Aider and others) look for before touching a repo. For a dev-tool this is a discovery surface as much as a contributor doc: the agents reading this repository are also the readers who recommend tools. It carries what an agent cannot infer — that `npm test` is the gate and `.githooks/pre-commit` runs it with no bypass, that there is no build step and no CI workflow, that a new suite must be wired into the `test` chain by hand or it silently never runs, and the hard nevers (no publish/tag/push, no paid API call to try something out, no runtime dependencies, no behavioural mocks in tests). Style is not duplicated — it links `CODING_STANDARDS.md`. A companion E2E pins the file's load-bearing claims against the repo (every script it names exists, every link and mapped path resolves, the gate and zero-dependency claims still hold), because a stale instruction file is worse than none: a missing one makes an agent look around, a wrong one makes it confidently do the wrong thing. Not added to the published `files` list — it is about working ON the tool, not with it.

### Fixed

- **«N gaps found» counted questions nobody asked as competitive gaps — in the client's own document.** The Competitor Intelligence line read every cell that was not a hit as a gap, so the unasked cells of a declared subsample were published as places a competitor had taken the brand's slot. On a run with two real gaps and two unasked cells it printed **«4 gaps found»**; the matrix beside it was right, only the sentence lied, and the section renders in `renderWhiteLabelMarkdown` too. This is the fifth surface of the class the declared-subset work closed on four, and it has been live since the Claude leg went to a fixed 15-of-50 subsample on 2026-09-16. It now counts only cells that produced a measurement (`lib/score.js#isMeasuredMention`, the same one definition the other four use). A failed call is also not a gap — it never was — but it no longer shares its status string with a never-asked cell: both still render as a dash, and `'missing'` stopped being able to mean two things the moment the declared-subset import made it a real value. Found by code review, not by the suite: no test called that function with an unasked cell.

- **The exit code of `--strict-model-pin` was covered by a test file that did not exist.** `test/model-drift.test.js` pointed at `test/e2e/run-model-drift.test.js` as the coverage for the answer-surface wiring; nothing was there. For a tool whose contract with a CI guard *is* the exit status, that is the last place a coverage claim may be taken on trust. The E2E now drives a real run offline and asserts all three: the warning reaches the terminal, `requestedModel`/`resolvedModel`/`modelDrift` reach `_summary.json`, and the opt-in flag turns drift into exit 1 — **without downgrading** a more severe exit 2 (invisible) or 3 (all engines errored).
...
- **A comparison between two runs was keyed on a question's POSITION in the config, so it could subtract answers to two different questions and call the result movement.** `lib/diff.js` matched cells as `` `${r.query}|${r.provider}` `` where `r.query` is the ordinal label `Q12`. Reorder the basket, drop one question, add three, and `Q12` is a different question with the same name. On the two runs actually on disk for the webappski basket — 2026-08-27 (56 questions) and 2026-08-31 (50 questions) — that lined up 50 label pairs in which **all 50 texts differ**: every row of that comparison was two unrelated questions rendered as a gain or a loss, and the headline `1% → 13%` was a change of basket presented as a change of visibility. **Identity is now the question itself**: `lib/basket-manifest.js` derives a stable id from the normalised text (and reads the market the config DECLARES for it, never one guessed from the answer), `run` stamps `queryId` on every cell it writes, and legacy snapshots are matched through the `queryText` they already carry — so the fix reaches runs recorded before it existed. Questions present in only one run are **counted and named** as incomparable instead of being mis-paired or dropped silently. When the baskets differ, the overall `scoreDelta` is **withheld** (`null`, with `scoreDeltaReason: 'basket-changed'`) and replaced by a like-for-like delta computed strictly over the intersection and stamped with how many questions it rests on. On that real pair: 31 comparable questions, 19 asked only now, 25 retired, and a like-for-like move of **+7pp (1% → 8%)** rather than the +12pp the headline implied. A changed basket never trips the regression exit code.

- **The same defect lived in the client's document, and that is where it did the damage.** Fixing `lib/diff.js` fixed the CLI; the report is built by other code, which paired cells the same wrong way. `sectionDiff` (the **What Changed** table) looked the previous run up with `p.query === r.query && p.provider === r.provider`, and `comparison-segments.js#cellKey` — the base of `run-comparison.js`, which its own documentation calls the single source of truth for the run verdict — keyed cells as `${query}::${provider}`. On the real 2026-08-27 → 2026-08-31 pair that table printed **twenty green "no → yes" rows**, each one two different questions sharing an ordinal label, and it ships inside `renderWhiteLabelMarkdown` — the paid client deliverable. Both surfaces now key on the question (`lib/basket-manifest.js`), and both agree with `lib/diff.js` on the same pair: **six** real changes, asserted against each other by a test so the two can never tell two stories again. The table names the question text beside the label, and states in the client's own document how many questions each run asked alone. `segmentCells` gained an `incomparable` segment — a question only one run asked is never a gain, a loss or a "never", and its size is published in `counts.incomparable` / `counts.incomparableQuestions`. `findBlankQueries` ("never held in either run") is likewise computed per question rather than per slot. The **Trend per Query** sparklines were keyed the same way and drew one continuous line through whichever questions occupied a slot over the weeks — they are now one line per question, with a gap in the runs that did not ask it. The run-comparison section gained a sized note (`counts.incomparableQuestions` / `counts.incomparable`) stating how many questions only one run asked, and its «visibility held steady» message is now disqualified by a basket change — without that, a report that rewrote every question would have rendered as a quiet week, because incomparable cells keep lost and gained at zero. The basket axis is now reported everywhere the other two comparison caveats are: an alert card in the HTML report (`buildBasketCaveat`), the note in the markdown section, and `basketChange {changed, questions, cells}` in the Mission Control payload beside `modelChange` and `coverageChange` — the payload publishes a UVI delta, and that delta moves by basket composition alone. The per-engine coverage figure deliberately does NOT absorb those cells: «this engine accounts for N answers» counts only the shared questions it stopped answering, or a dropped engine would be credited with a basket change it had nothing to do with. `lib/report/answer-history.js` deliberately still keys on the SLOT: a history of a tracked slot is a different question from a comparison, and it surfaces a reworded question as `textDrift` (rendered in both report surfaces) instead of restarting the record — that divergence is now stated in its own docstring, which until today claimed the opposite.

- **One denominator for the whole report, in one module (`lib/score.js`).** The same question — *out of how many?* — was answered in five places with three different answers: the headline score excluded errored calls, the per-engine rate, the lift aggregate and the funnel/intent table included them, and the HTML matrix counted its own cells again. A run with API errors therefore published several different denominators for one basket, with nothing saying which one the headline used. Separately, a cell measured with `--samples N>1` already stored its own fraction (`presence.hits/n`) and every score counted its **representative label** instead — so a cell measured twice as yes/no counted whole, reporting 100 where the measurement said 50. The run now publishes four numbers with one meaning each — **hits · valid · attempts · errors** (`measurementCounts` in `_summary.json`, and a printed line whenever they can differ) — and every surface that shows a share divides by `valid`. `mentions`/`total` remain as the names consumers read, now as aliases of that one computation rather than a second count of it. A grep-based guard test fails the build if a further surface starts counting hits on its own — in either shape: the inline `yes || src` test, and the branched `if (yes) … else if (src) …` form beside a self-kept total, which is how a fourteenth divergence (the `coverage` object feeding the HTML engine card's «Named in X/Y cells») survived the first version of the guard. Two smaller consequences, stated because they change what a reader sees: an engine whose every call failed now shows **no** previous-run percentage instead of `0%`, so the engine card suppresses the delta rather than drawing a jump out of a fabricated zero; and a cell nobody asked (`mention: 'missing'`, the matrix placeholder) counts as neither an attempt nor a failure, so a sparse matrix no longer reports calls that were never made as calls that failed.

  **This did not change any published number for the 2026-08-31 baseline, and nothing here says it did.** That run had zero errors and no repeated trials, so neither defect could fire on it: it re-scores to exactly **13** under the new formula, which a fixture asserts on the real records. What was broken was comparisons, and any future run that samples or errors.

- **The basket knows which directory it belongs to (`basketManifest.runRoot`).** `aeo-responses/<domain>/<date>/` resolves relative to the current working directory, so the same basket run from two directories builds two parallel histories under the same dates and "compare with the baseline" silently depends on where the shell is standing — which is how two byte-identical 2026-08-31 summaries came to exist in two trees. `run`, `report` and `diff` now say out loud when the current directory is not the canonical one. **Nothing is moved, redirected or deleted** — the fork is made visible, which is the half that was doing the damage. A config can also be marked `_deprecated` (with the canonical one named) instead of being deleted; `run` prints that before spending anything.
...
- **Gemini default bumped to the newest generation.** `gemini-3.7-flash` → **`gemini-3.8-flash`** in `DEFAULT_CONFIG` (`lib/config.js`), the discovery `FALLBACK` (`lib/providers/discover.js`), the pricing table (`lib/providers/pricing.js` — same $0.75/$3.75 per 1M rate as 3.7, in force through 2026-12-31), and every README mention. Verified present on a live key via `discoverModels('gemini', …)` the same day, returning exactly `['gemini-3.8-flash']`.
- **New env-var pins for automated callers: `AEO_OPENAI_MODEL_PIN` / `AEO_GEMINI_MODEL_PIN` / `AEO_ANTHROPIC_MODEL_PIN` / `AEO_PERPLEXITY_MODEL_PIN`.** `--<provider>-model` already outranked discovery for a single invocation; a cron or an agent has no flag to type, so the pin had no durable form. The env var feeds the same `cliModelPins()` precedence (CLI flag still wins if both are set) — no change to discovery's own behavior or to any existing flag/test.
  Immediate reason: the same live-key check found OpenAI generation 6 has shipped exactly one model, `gpt-6-astra`, at **$10/$50 per 1M tokens** (~50× `gpt-5.6-luna`'s $0.20/$1.20) with no recognised cheap-tier name — discovery's tested, intentional "newest generation always wins" rule (2026-09-01 ruling) would pick it and pay flagship rates, with only a stderr warning. `AEO_OPENAI_MODEL_PIN=gpt-5.6-luna` holds our own runs on the cheap tier until a real gpt-6 cheap variant exists (then either drop the pin or add its name to `MAIN_CHEAP_TIER`). The default (unpinned) behavior for every other installer of this package is unchanged — this is an opt-in escape hatch, not a policy reversal.

> **Maintainer note (updated 2026-09-07), surfaced rather than silently resolved.** Everything still in this section shipped in **1.2.x–1.12.0** and is filed here only because nobody has split it yet — it is backlog, not a preview of the next release. npm serves **1.12.1** [verified live 2026-09-07 · `npm view aeo-platform version`, re-derived later the same day: 1.12.1 was published between the two checks], and the newest dated header below is `## [1.12.2]`, which is written but **not yet published** — `package.json` still reads 1.12.1. The split IS derivable — every entry has a commit (`git log --date=short -- CHANGELOG.md`) and every release has a publish date (`npm view aeo-platform time --json`), so an entry belongs to the first version published after its commit. June–August maps cleanly that way; **2026-09-01/02 does not** — 1.10.0, 1.11.0 and 1.12.0 all published inside 48 hours, so entries from those two days cannot be attributed without guessing, and guessing here relabels someone else's release. Whoever does the split should mark that cluster ambiguous rather than inventing precision. 1.12.1 was promoted out on 2026-09-07 because its scope was unambiguous (three docs commits, none of them published before).

### Changed

- **The tracker measured on a two-generation-old model and called it discovery.** OpenAI's 5.6 line replaced size suffixes with names (`sol` / `terra` / `luna`), and the selection rule was `const pool = minis.length > 0 ? minis : filtered` — a `-mini` FILTER applied *before* the generation sort. Nothing in the newest generation matched, the entire generation fell out of the pool, and discovery reported a healthy pick of `gpt-5.4-mini` while the world had moved on. Verified live 2026-09-01 with `AEO_DEBUG_DISCOVERY=1`: of 124 ids on the key, 8 reached the pool and every one carried `-mini`. **The rule is now generation-first: the newest generation on the key IS the pool, and a cheap-tier preference chooses inside it.** Cheapness may no longer cost a generation. The preference list is a hint, not a gate — when the newest generation matches none of it the fetcher says so on stderr and names the file to edit, so the next vendor rename surfaces in one run instead of in three months of drifting numbers. Defaults bumped to the newest ids verified present on a live key that day: OpenAI `gpt-5-mini` → **`gpt-5.6-luna`** (newest generation, cheapest tier — $0.20/$1.20 per 1M), Gemini `gemini-3.5-flash` → **`gemini-3.7-flash`**, in both `DEFAULT_CONFIG` and the discovery `FALLBACK`. Founder ruling 2026-09-01: a measurement exists to show what the engines answer **today**; a frozen SKU measures a world that no longer exists.

  **Comparability, stated plainly: your next run measures on a different instrument than your last one, and the two are not a like-for-like pair.** Discovery already moved you forward silently in the past; what changes now is that the run-comparison will SAY so — the model axis below flags the swap and qualifies the movement instead of attributing it to your visibility. A drop or jump on the first run after upgrading is at least partly the instrument. Pin the previous id (`--openai-model=<old id>`, which now actually works) if you need one more like-for-like data point before moving the timeline over.

- **`--openai-model` and its siblings were documented no-ops.** `--help` promised *"Override providers.openai.model for this run"*. The flag was written into `config.providers.<name>.model`, and the run loop then resolved `r.models ?? (r.cfg.model ? [r.cfg.model] : null)` — so on any run where discovery SUCCEEDED, the `??` never reached the config value and the flag did nothing. It worked in exactly one case: when discovery had already failed. This cost a paid TypelessForm run on 2026-09-01, which returned byte-identical results to the un-flagged run before it. **A pin now outranks discovery**, which is the entire point of it — re-measuring an earlier month like-for-like means running the id that month ran, not the id that is newest today. The precedence (pin → discovery → config file) lives in one pure, unit-tested function rather than in an operator inside a loop, and the run log now names which of the three the model came from. **A pin the provider's catalogue no longer lists stops the run** with the available ids listed, before the first billed call — validated against the RAW catalogue, so `gpt-5-search-api` (deliberately excluded from the selection pool, and exactly the id you pin for a like-for-like re-measure) stays pinnable. An unreadable catalogue is "unknown", not "absent", and proceeds.

- **The comparison engine guarded the question axis and was blind to the instrument.** A shifted question basket already makes `coverageAllowsDelta` decline to state a delta; a swapped MODEL passed straight through and read as the brand moving. [Corrected 2026-09-15, without rewriting what this entry shipped: that first clause claims more than the gate does. `coverageAllowsDelta` tests the coverage RATIO, so it declines on a basket that changed SIZE or went partly unmeasured — and passes a same-size swap of one question set for another, which it cannot see. Question identity was not checked anywhere until the 2026-09-15 entry at the top of this file; the guard described here was, and remains, the instrument axis.] What that produced, on a real client report: *"Your visibility index moved from 87 to 69 — down 18 points"*, followed by a "Where you lost ground" table naming six competitors as having taken the space — while four of its five rows had been measured on a different model than the run they were compared against (`gpt-5-search-api` → `gpt-5.4-mini`, `gemini-3.5-flash` → `gemini-3.7-flash`). The newer OpenAI SKU had read the e-commerce question as being about browser extensions and listed those. Every link looked plausible; none of it was a fact about the brand. **The fix is attribution, never a gate** (founder ruling 2026-09-01: «никак мы не можем не сравнивать его движение пользователя») — the delta is still computed and still shown in full, and the swap is named beside it. New `lib/report/model-change.js` classifies the step (`snapshot` < `minor` < `generation` / `line` / `surface`, plus an honest `unknown` when two ids cannot be related — a missing version number is never reported as a generation change), calibrated on a same-day A/B: a product-line swap flipped 2 of 3 questions and cut answer length ~3× (STRONG), a minor version step moved mention density while every yes/no verdict held (MODERATE). The caveat lands **inside** the block that names competitors — per row, with a count of how many of that table's own rows are affected and the actual «was → is» ids — because a footnote does not reach a reader who has already read the names. "No overlap at all between the two runs" is reported as its own state, not folded into "changed". Also in the Mission Control payload as `modelChange`.

- **Every cell now records which model produced it; the summary says when its cost total is incomplete.** `_summary.json` cells carried `model` (requested) and stamped a `requestedModel`/`resolvedModel` pair *only* when the served model was a different lineage — so for a normal cell the id the provider actually served, dated snapshot and all, was unrecoverable. Catching the swap above meant opening raw response files and diffing filenames by hand. Provenance is now unconditional (`requestedModel` + `resolvedModel`), with the drift FLAG moved to its own `modelDrift` field, and the comparison's model axis reads it instead of guessing from a string. Separately: discovery walks itself to each vendor's newest generation while the pricing table is hand-maintained, so discovery outruns pricing **by construction** at every generation change — 2026-09-01 the run printed *"cost not tracked for: gemini-3.7-flash"* while `sessionCostUsd` sat in the summary understated with nothing saying so. The total is not fudged with a guessed price (this table once mispriced a default by ~20× on exactly that instinct); runs with an unpriced model now carry `costComplete: false` and `costUntrackedModels`.

- **Both legs of one run were graded by different models.** `run` handed the extractor a freshly-discovered classify tier; `run-manual` handed it the config file verbatim. On 2026-09-01 that split a single day's twelve cells between `gemini-3.1-flash-lite + gpt-4o-mini` (API leg) and `gemini-3.5-flash + gpt-5-nano` (pasted legs) — cells compared against each other, judged by different graders, with nothing in the artifact saying so. `run-manual` now resolves the classify tier the same way `run` does, and both writers record `extractorModels` in the summary so a residual difference (two commands, minutes or days apart, a vendor rolling a model between them) is knowable from the artifact instead of from two terminal scrollbacks.

- **Gemini flash cells were billed at roughly twice their real rate, and the price checker was validating against next year.** `gemini-3.6-flash` carried $1.50/$7.50 per 1M — Google's published rate is *"$0.75 through December 31, 2026. $1.50 starting January 1, 2027"* (re-verified 2026-09-01 on `ai.google.dev/gemini-api/docs/pricing` and in the `simonw/llm-prices` feed), so the table held the post-promotional number as if it were current. `scripts/pricing-check.mjs` did not catch it because it selected the history entry with `to_date === null` — which is precisely where a scheduled future rise lives. It now picks the entry whose window contains today. Corrected `gemini-3.6-flash`, added `gemini-3.7-flash` (previously unpriced — the cause of the untracked-cost line above) and the `gpt-5.5` / `gpt-5.6` rows. Revisit both flash rows on 2027-01-01. Same class, Anthropic side: the broad `claude-sonnet` row charged **$3/$15** to `claude-sonnet-5`, the default answer model, which is **$2/$10** — Anthropic's docs state the launch introductory price *"is now the standard price"* and that *"the previously scheduled increase to $3/$15 per million input/output tokens on September 1, 2026 will not occur"*, so the row was not stale but encoding a cancelled rise. Sonnet 5 gets its own row; the family row stays $3/$15, which is still correct for Sonnet 4.x. `npm run pricing:check` was already failing on this before any of the work above and now passes.
...
- **`--regions pl,at,ch` — Poland, Austria and Switzerland are now real markets, so the region axis can finally aim at the PL/DACH beachhead it was built for.** `REGIONS` shipped 12 codes and none of them were these three, while the localisation table `LANG_MARKET` had *already* carried localised market names for exactly `pl`/`at`/`ch` — unreachable dead code, because `parseGeoFlag` rejected the codes outright. Adding them to `REGIONS` alone is not sufficient: a third map, `REGION_NATIVE_LANG`, decides which language a market is asked in when SEVERAL are requested, and a region missing from it silently falls through to whichever language was listed first. So `--regions pl,de,at,ch --lang pl,de` would have asked the Austrian and Swiss cells **in Polish** while still exiting 0 and still tagging the regions correctly — nothing but the `lang` field would have shown it. That branch had zero coverage because every prior test passed a single `--lang`, which short-circuits before the map is read. All three maps now grow together, pinned by a test that fails if a region resolves to a different language when the `--lang` list is rotated. Same change makes `--help` render the region and language lists from `listRegionCodes()` / `listLangCodes()` instead of two hand-typed lists that had already drifted out of sync with the code. **Semver: this is a new backwards-compatible capability — `npm version minor` is the appropriate bump** (per the R11 release table). A run that never passes `--regions` is byte-identical to before: the axis is opt-in and leaves neither a `region` nor a `lang` field on any cell.

- **README section «How we count visibility» — one public page stating the rules behind the number, including the one thing the README never said: how many times we ask.** Competitors publish a methodology page as the price of entry to the category; we had four scattered pieces of one (`What this measures — and what it does NOT`, the crawlability audit, authority signals, `UVI methodology`) and a hole in the middle. Grep-confirmed before writing: across all 1,032 lines the README never contained the strings `--samples`, `Wilson`, or `confidence interval`, and never defined what counts as a mention — line 370 used the bare jargon `(yes/src)` in the UVI table without ever saying what those labels mean. The new section states it outright rather than in general terms: **`aeo-platform run` makes exactly one API call per cell** (a cell = query × engine × region × pass), so a default run is a **point estimate with unknown spread**, two runs the same day can honestly disagree, and one cell flipping is not a trend; `--samples N` (default 1, cap `MAX_SAMPLES` = 25) repeats each cell and reports a 95% Wilson interval that `diff` uses to call a change noise. It then writes down the mention taxonomy (`yes` body / `src` citation-URL only / `no` absent) and says the least obvious scoring decision in the tool out loud — **`yes` and `src` both count as one for Presence** — with three matching limits named rather than hidden (no fuzzy matching, so an engine's misspelling is an absence until an alias is configured; the domain is matched as a plain substring without word anchoring; a dot is significant, `nodejs` does not match `Node.js`). Per-axis derivations follow (errored cells dropped from the denominator, not scored as absences; low-confidence neutral sentiment dropped rather than averaged in as a 50; prose ordinals discounted ×0.7 against explicit list ranks; an unmeasurable axis excluded with the remaining weights re-normalised, never zero-filled; citations matched at the registered-domain level so subdomains count), plus which model actually answered (IDs are discovered live; config values are fallbacks; the served ID is stamped in `_summary.json`) and four things the tool never claims to know (real query volume, classic Google rankings, causation, tomorrow's answer). Nothing from the neighbouring sections is restated — they are linked, so there is one home per fact. `--samples` also lands in both flag lists, which had never mentioned it.

  **Two pre-existing inaccuracies in the UVI section were corrected in passing rather than left to contradict the new text:** the weights table and its summary line described Sentiment as *"high-confidence positive cells out of cells with a mention"*, while `computeComponents` actually averages the signal-bearing cells at `positive` 100 · `neutral` 50 · `negative` 0 (excluding failed/empty labels and low-confidence neutrals); and the Presence denominator is non-error cells, not all cells.

  **Guarded, not asserted:** `test/e2e/methodology-doc-truth.test.js` pins every load-bearing claim against the real modules — the sampling default and cap against `resolveSamples`/`MAX_SAMPLES`, the published weights table against `computeUVIBreakdown` (previously unguarded prose, so this closes a pre-existing drift hole), the documented prose discount against `PROSE_RANK_DISCOUNT`, the mention taxonomy against `detectMention` (including the separator-tolerant, word-anchored, dot-significant and no-fuzzy cases), the yes/src presence rule against `perCellPresence`, error-cell exclusion and axis re-normalisation against `computeComponents`, and the citation rule against `isOwnDomain`. Proven red in both directions: mutating `MAX_SAMPLES`, the presence weight or the prose discount fails the file, and weakening the README's run-count sentence fails it too. Docs + test only — no product code, no score math touched.
...
- **The model-axis guard shipped in 1.11.0 was wired to the markdown report only. The HTML report — the file that gets opened, printed and handed to a client — built the comparison model into a variable and never read it.** `lib/report/html.js` computed `const comparison = buildRunComparison(snaps)`; `grep -n comparison` on that file returned the import, that assignment, and one unrelated word inside a string literal. Zero reads. So the caveat the previous release added, the one that names the model swap and qualifies the movement, reached exactly one surface — and not the one the 2026-09-01 false story was actually read from. That report said *"scored 50 of 100 — 42 points below the 2026-08-13 run"* and named the companies that had taken the brand's place, with nothing anywhere on the page saying ChatGPT had answered the two runs on two different models. The portal payload carried `modelChange` and the portal did not render it either. **The caveat copy now lives in `lib/report/instrument-caveat.js` as sentences rather than markup, and both surfaces render the same words** — markdown italicises them, HTML puts them in an alert card **leading the Overview section, above the block that names competitors**, because a note underneath does not reach a reader who has already read the names (the same rule the markdown lost-table already followed with its per-row mark). The delta is still stated in full, unchanged: the fix is attribution, never a gate. Markdown output is byte-identical except one closing sentence — *"the movement above"* → *"this run's movement"* — which had to stop describing a position it no longer holds on both surfaces.

- **An engine that was measured last run and skipped this run left no trace in any sentence of any report — and could produce a clean bill of health while doing it.** `buildModelChanges` correctly declines to call a one-sided provider an instrument swap (it is a coverage change, and calling it a swap would double-count what the segment model already handles). Nothing then reported the coverage side: those cells land in `segments.indeterminate`, whose count reached the Mission Control payload and no renderer at all. Because indeterminate cells keep `lost` and `gained` at zero, **a run that quietly stopped measuring an engine was the exact shape that triggered *"No material change since the last run — visibility held steady across every tracked question and engine"*** — a sentence that is false in its own terms the moment an engine went unmeasured. Dropped and added engines are now named, with the number of answers they take out of the won/held/lost tally, that message no longer fires on a coverage change, and the payload carries `coverageChange` alongside `modelChange`. What the new copy deliberately does **not** claim: that the score itself is scoped to the shared engines. It is not — each run is scored over that run's own answers, so a coverage change moves the delta by basket composition alone (on the committed fixture: −18 with all four engines, −21 with one dropped, −17 with one added; all three asserted by test). The score gets the same hedge the instrument caveat gives it. A first draft of that paragraph asserted the exclusion instead, and review disproved it before it shipped.

- **The identity-links ("Fixable in one line") alert card compared an edge's status against the wrong casing and, once fixed, was found to also mislabel a POSITIVE signal as broken — caught live on webappski.com's own report.** `entity-graph.js`'s `verifyEdge()` returns kebab-case statuses (`'one-way'`, `'broken-link'`, `'verified-host'`, `'unreachable'`, `'reciprocates'`), but the render call site in `lib/report/html.js` compared `e.status` against camelCase literals (`'oneWay'`, `'brokenLink'`) that can never match — so every one-way and broken-link edge silently fell through to the generic "Not verified" pill, and broken-link edges were mistinted `warn` (orange) instead of `bad` (red). Root-caused live: 12 sameAs URLs on webappski.com's own schema, the exact "9 of your 12 identity links do not resolve" headline from the report. The status→label/tone mapping is now a single exported function, `describeEdgeStatus()` in `entity-graph.js`, used by the one render call site instead of an inline ternary. **A second, sharper defect surfaced in review of the first fix, before either shipped:** the card's `broken` bucket (drives the headline count, the "returns X to a crawler" sentence, and which edge is picked as the "fix this" `sameAs` code sample) counted `'verified-host'` edges as broken — even though `summariseEdges()` already treats `verified-host` as a POSITIVE signal in `reciprocityRate`, same as `reciprocates`, because it is the best available signal for an auth-walled platform (LinkedIn, Medium — see `AUTH_WALL_HOSTS`) we cannot reverse-check. Un-caught, the label fix alone would have made the card **worse**: a green "Verified host" pill rendering directly under a "9 of 12 do not resolve" headline, body copy stating those links "return … verified-host, verified-host, verified-host … to a crawler" (a raw internal status string interpolated as if it were a fetch error), and the code sample instructing the client to "fix" `https://www.linkedin.com/company/web-appski/` — a live, working, correctly-declared LinkedIn page. `broken` now excludes `verified-host` alongside `reciprocates`; on webappski.com's own live data this drops the headline from the misleading "9 of 12" to the accurate "6 of 12" (3 reciprocate + 3 verified-host = 6 genuinely resolve or are host-confirmed; 3 one-way + 2–3 unreachable are the real gap), and the "fix this" sample now targets the first genuinely broken edge instead of an auth-walled one. Regression-tested at two levels: `test/entity-graph.test.js` pins `describeEdgeStatus()` for all 5 canonical statuses + an unknown-status fallback (that test file existed before this fix but was never wired into any npm script — now `test:entity-graph`, slotted into the `test` aggregate chain); `test/html-render-smoke.js` adds 4 tests against the actual `renderHtml()` call site — the exact place both defects lived — asserting the broken-count excludes verified-host, no raw status word leaks into client-facing copy, the code sample never targets a verified-host edge, and every tile carries its correct worded label + tone.

- **Four of the localised query preambles asked the engine in broken grammar — a doubled preposition, on the exact markets the region axis exists to reach.** `--lang fr --regions ch` sent, verbatim: *"(Réponds dans le contexte **de du** marché suisse. Réponds en français.)"* — `de` from the sentence template plus `du` (= `de`+`le`) from the market phrase, two prepositions where French allows one. Italian read `di del`, Spanish `de del`, Portuguese `de do`. The defect class predates the region axis, but it was unreachable dead code until `pl`/`at`/`ch` became real regions, which both added two new instances (`fr→ch`, `it→ch`) and made the whole class live. Fixed once as a class rather than five times: **the sentence template now supplies no grammar for the market slot, and the market phrase carries its own.** That is the only shape that generalises across the shipped languages — French/Spanish/Italian/Portuguese contract the preposition with the article, German inflects for case (`von` + dative → `dem deutschen Markt`, which no template can supply), Polish needs no preposition at all (`w kontekście` governs the genitive directly), and Japanese puts the slot first. Simply deleting the preposition from the four templates would have traded a doubled preposition for a **missing** one on every untranslated pair, where the English market name is spliced in; those now get the language's own preposition from a dedicated fallback table and render exactly as before. **126 of the 135 (language × region) cells are byte-identical — the 9 that moved are precisely the broken ones.** Every cell is now pinned as a full string by an exhaustive golden table, plus a sweep that flags two adjacent prepositions in any rendered cell; the assertions that let this ship were substring checks (`.includes('du marché suisse')`), which pass just as readily on `de du marché suisse`, because a substring assertion cannot see the words *around* the needle — which is where prompt grammar lives.

  **Comparability (R39): only newly-reachable cells moved.** `de→de` deliberately keeps the slightly stiff `von dem deutschen Markt` rather than the idiomatic `vom` — it is the one localised cell that appears in historical measurement runs, and changing its prompt would break comparison against them. `de→at` and `de→ch` ship the correct `vom` because they became reachable this week and have no history to protect. That asymmetry is intentional and documented next to the table. No run recorded before this release used French, Italian, Spanish or Portuguese, so those fixes cost no comparability at all.

- **The publish smoke only ever ran one of the two `bin` aliases.** `package.json` maps both `aeo-platform` (current) and `aeo-tracker` (the legacy name every install predating the rename still invokes), but `smoke-tarball.sh` read `Object.keys(bin)[0]` — so a broken mapping on the legacy alias would have passed `prepublishOnly` and reached the registry. It now packs the tarball, installs it into a clean prefix and asserts `--version` (matching `package.json`) and `--help` for **every** declared alias. Install is a never-fail surface; a dead alias is a dead install for anyone still typing the old name.

- **`npm version` could not complete a release — the README version pin introduced in this cycle had itself become a release trap.** The README's Schema.org `softwareVersion` is pinned to `package.json` by a test, and `.githooks/pre-commit` runs the full suite with no bypass. `npm version minor` bumps `package.json` and commits in one step — which meant the bump landed, the hook ran, and the release died on a README field the maintainer had no chance to update. Confirmed by simulation, then closed by wiring `scripts/sync-readme-version.mjs` as npm's `version` lifecycle script: it rewrites the README's `softwareVersion`/`dateModified` and stages them, so npm commits the corrected README inside the same release commit. **Maintainer-visible change to the release flow: `npm version <x>` now also touches README.md — expected, not a stray edit.** The pin keeps its teeth: a hand-edited README that disagrees with `package.json` still goes red.

- **The report's empty-sentiment panel published a number it had never measured.** The panel printed the literal string "0 named cells" while the CSS gate that decided whether to show it counted something else entirely — so on a run where the brand *was* named, the report told the reader zero. The count and the eligibility rule now both derive from the run, and agree with the classifier that produced the labels.

- **The published Schema.org entity described a package that no longer shipped.** README `softwareVersion` read 1.1.4 while npm served 1.8.0 (six releases stale), `dateModified` was absent, and the `Organization` node's `@id` was `#org` while the live canonical site publishes `#webappski-org` — so the two nodes never merged and a consuming engine saw two unrelated organisations. The `@id` is now aligned to the live value (verified by fetching both domains, not from memory) along with the `sameAs` list that node publishes about itself. The six competitor-parity stamps were re-anchored to the **July 2026** review of 23 tools that actually ran, and deliberately **not** bumped to August: the August scouts logged new entrants but did not re-check capability parity, so an August stamp would have claimed a scan that never happened.
...
- **The "Score over time" chart garbled its own labels on any report where an early run was partial and a late run scored near 0 — exactly the shape of a real client report (webappski.com, 2026-08-27, score 1).** Two independent SVG text collisions, both against the same design flaw: a "partial" flag was drawn as a separate `<text>` over **every** partial-run point (up to 8 of them, one per run), and the last-value label and the day-1 baseline note were each anchored a fixed distance from their own point. When the score sat near 0, the last-value label (`y = lastY + 26`) landed almost exactly on the date axis's fixed `y=224`, rendering the final date and the final score as one illegible glyph; when the first run was partial, its "partial" flag sat right where the day-1 note's own text already was. Fixed on both axes: (1) the per-point flag is gone — one shared callout now marks each *contiguous* run of partial points, drawn once near the top of the chart where nothing else lives, and each point keeps its own detail as a hover `<title>` instead; (2) the day-1 note and last-value label no longer sit a fixed distance from their point — `openSideY()` puts each on whichever side (above/below) is actually open, which is below when the point's own score pushes it toward the top of the plot (clear of the new callout) and above when it sits toward the bottom (clear of the fixed-y axis row). The accompanying "Run N measured X of Y answers…" caption, which previously repeated one near-identical sentence per partial run (8 sentences in the real report), now collapses consecutive same-coverage runs into one row of a compact table. Regression guards: `test/loud-components.test.js` pins the collision fix directly at every affected boundary (low score, and — caught in review — a partial run *itself* scoring near 100, which the first fix pass missed); `test/html-render-smoke.js` pins the collapsed-table behavior end to end. Rendering only — no score, axis, or UVI math changed; regenerating an older report reflows the same numbers into the corrected layout.
...
- **The comparison table now carries the row for the agency that publishes it.** `## Comparison vs hosted AEO platforms` graded seventeen other companies and left out Webappski, the one company a reader can actually hold this repository to. That is now a row, alphabetically last, labelled in the cell as *agency — the maintainer of this project*. Category fit was never the issue and is settled inside the table itself: **Discovered Labs** (`Managed-service retainer`) and **Evertune** (`Custom contract`) are agencies, not self-serve tools, and both have held rows since May.

  **What the row says, cell by cell, and how each cell was checked.** *Pricing model* — `Monthly retainer or one-off project, invoiced`, the shape published on webappski.com/en/pricing, read on 7 September 2026. The value first proposed for this cell was "project-based engagement", which is **half the truth**: the same page sells four monthly tiers alongside the one-off work, and a cell that names only the one-off path would have understated our own commercial shape in the direction that flatters us. No price appears, here or anywhere in this README — the pre-publish lint that treats `$N/mo` as a commerce CTA is the mechanical half of that rule, and the intent is the other half. *Open source* — `MIT — the same engine as row 1`, true without qualification, because the agency delivers with this package. *Raw data stays local* — leads with **No**: the free audit on webappski.com executes in Firebase Cloud Functions on Webappski's own provider keys (`functions/src/free-audit/engine-probe.ts`, `run-free-audit.ts`, read live), and an audit we deliver is run by us on our side, so on both delivered paths the raw answers land in our infrastructure, not on the client's disk. The cell leads with the No deliberately: a reader of that row is weighing *engaging us*, and for them the answer is No; the Yes belongs to the CLI path, which is row one. *Portable paste-into-AI plan* — `Yes`, same engine, same export; that the paid deliverable also contains hand-written work adds to the export rather than replacing it.

  **A row of your own falsifies sentences a competitive table never had to think about.** Four of them, all repaired in this commit: our own Portable cell read *"the only row in this table"* (now *"the only engine in this table that ships one"*); the transparency paragraph read *"`aeo-platform` is the one MIT row in it"* (now *"the only MIT in it is this engine, on the two rows that run it"*); and the total *"18 tools in that table"* appeared in both the README FAQ and the Schema.org FAQ answer, which now decompose it instead — **none of the 17 third-party products** shipped a portable plan at the 4 September re-check, and the **two of 19 rows** that do ship one are both ours. The wedge heading and its anchor are unchanged, and the note under the table now states in words what the heading assumes: *no hosted vendor ships a portable plan* is a claim about the seventeen third-party products, and the two Yes cells trace to a single engine.

  **The live re-check stamp did not move, and that is the point.** It still reads *"a live re-check of 19 tools on 4 September 2026"*, still decomposes as 8 ✳ hosted + 8 open-source peers + 3 explicitly-excluded projects, and the ✳ note now says explicitly that the mark applies to third-party rows only. Our own rows carry no ✳: the mark means *we read the vendor's linked page live*, which is not a claim anyone can make about themselves, and a ✳ on our row would have inflated an audited number by counting us as a competitor we had checked.

  **Three new pins in `test/e2e/readme-comparison-truth.test.js`, all ownership-aware.** No row of ours may carry a ✳; every counted claim about the hosted table (`N rows in that table`, `N third-party products in that table`) must equal what the markup holds **on every surface that repeats it**, prose and JSON-LD together, so one healed surface can no longer hide a stale one; and a Yes in the Portable column may appear only on a row we own — the day a real vendor ships a portable plan, that assertion fails and forces the wedge prose to be rewritten in the same commit instead of quietly becoming false. Ownership is derived from the row's own href (`github.com/webappski/` or `webappski.com`), not from a hard-coded name list, so a rename cannot orphan the check. Each pin was mutation-tested RED before commit: ✳ added to our row, one `19` edited to `18` on a single surface, and a competitor's Portable cell flipped to Yes.

  **The row created a collision between two nineteens, and independent review caught it.** Until this release the hosted table held 18 rows while the stamp in the next section read *"a live re-check of 19 tools on 4 September 2026"* — two different numbers, impossible to conflate. The table now holds 19 rows too, and they are a **different nineteen**: the table counts 17 third-party products plus our own two rows, while the stamp counts tools whose own pages were read that day (8 ✳ hosted + 8 open-source peers + 3 excluded projects) and contains neither of our rows. A reader skimming could have carried away "they re-checked the whole table live, themselves included" — the precise over-reading the ✳ convention exists to prevent. Both places now say so in one clause: the note under the hosted table names the stamp as a different nineteen, and the stamp says it counts tools whose pages we read rather than rows in a table.

  **One sleeping defect in the older pin, fixed while the reasoning was fresh.** The re-check total derives "how many open-source peers did we check" as *every row of that table except our own*, and it identified our own row **by name** (`!r.name.includes('aeo-platform')`) while the three new pins identify ownership **by href**. Harmless today, since our only open-source row is called `aeo-platform`. It stops being harmless the moment a second row of ours lands in that table — precisely the move this release just made in the hosted one — or the moment that row is renamed: a name filter counts it as a peer we audited and silently raises the required re-check total by one. Now read from the href like everything else, and proven by differential test: rename the row's link text while keeping its URL, and the old filter goes RED while the new one stays correct. The json-fence strip in the surface split was also reordered to happen before the section is subtracted rather than after, so a fenced json block landing inside the hosted section can never blur two surfaces into one.

  **Correcting the record in 1.12.1 below, not just the table.** That release note explains, at length, why no Webappski row was added — and the reasoning was wrong on its own terms: it weighed only a `Yes/Yes/Yes` row (which would indeed have been false) against omission, and never tried a truthful row. Worse, the omission was filed internally as a founder decision when the founder had asked *how to write it honestly*, not whether to write it at all. A recommendation nobody answered is not a ruling. The paragraph below is kept verbatim, with a pointer, because deleting the wrong reasoning would delete the evidence of how it happened.

## [1.12.1] — 2026-09-07

### Changed

- **The competitor tables had not moved since 14 May 2026 and had started to describe a market that no longer exists.** `git log -S` puts the last substantive edit to `## Comparison vs hosted AEO platforms` at 2026-05-14 — nearly four months during which the internal competitor log picked up entrants that never reached the README. Both comparison sections were rebuilt on **4 September 2026** against live sources only: vendor pages via HTTP for the hosted side, `gh api repos/<owner>/<repo>` plus each project's own README for the open-source side, and `npm view` for registry claims. The cached competitor file was treated as a lead list, not as evidence.

  **Hosted table: 12 rows → 18.** Added [Waikay](https://waikay.io/) ($69.95 / $199.95 / $449.95 per month — the closest functional neighbour, since it sells *AIO Action Plans* by quota, 30 / 90 / 220 per tier), [Trakkr](https://trakkr.ai/), [LLMrefs](https://llmrefs.com/), [Superlines](https://www.superlines.io/), [Scrunch AI](https://scrunch.com/) and [AirOps](https://www.airops.com/). Two existing rows carried names the vendors have since retired: HubSpot's tool is now **AI Search Grader**, not "AEO Grader" (and HubSpot now also sells ongoing AEO monitoring, which the old free-scorecard cell implied did not exist), and Semrush's surface now reads **AI Visibility**. Otterly's cell said only "Paid subscription" while its site advertises entry pricing an order of magnitude below what our internal notes cached. Every row now links to a vendor page, and a ✳ marks the rows whose linked page was actually read on 4 September — so the reader can tell a re-checked fact from an inherited category, which the old table could not express.

  **The uniqueness claim was narrowed to what is true.** The wedge was stated as "no commercial vendor ships this". Waikay demonstrably ships *an action plan*; what no tracked vendor ships is a **portable** one — a prompt block you take to your own AI chat and keep after the subscription lapses. The column header, the section heading, both README FAQ answers and both Schema.org FAQ answers now all say *portable*, so the table and the machine-readable entity cannot contradict each other. The stamp is additive rather than replaced: "July 2026 review of 23 AEO tools, plus a live re-check of 19 tools on 4 September 2026", and the README **enumerates all 19** so the number can be audited instead of trusted. Re-stamping the July figure was considered and rejected for the same reason the August bump was rejected on 2026-08-27: a number nobody can decompose is a claim about work, not a record of it.

  **Open-source section: one named peer → a nine-row table.** It previously named geo-aeo-tracker alone. It now carries elmo, geo-aeo-tracker, getcito, oneglanse, canonry, ansvisor, lettertrace and sharozdawa/ai-visibility beside this project, with stars, licence, engine count and — the column that actually decides anything — what it takes to get a first number out of each. Ours is the smallest by stars (11 against elmo's 293) and that is printed rather than omitted; the differentiator claimed is the absent deployment step, not popularity. Licence precision matters here and was read from the LICENSE files rather than inferred: canonry ships **FSL-1.1-ALv2**, fair-source, whose own text grants Apache-2.0 only on the second anniversary of each release — it had been loosely filed as "open source". The GitHub API's `license` field is not sufficient for this: it reports `NOASSERTION` for canonry *and* for getcito, but getcito's `LICENSE.md` is plain **MIT** (it is a declared derivative work of elmo, which the classifier misses because of the extra copyright lines). A draft of this table shipped that `NOASSERTION` through as "no OSI licence declared" and was corrected before commit — an API field that says *I could not tell* is not a finding, and publishing it as one would have been a false claim about someone else's project. Three projects are excluded *with the reason printed*: addyosmani/agentic-seo (307 stars, but a site/docs auditor, not a mention tracker — this also resolves a contradiction in our own internal notes, which had it filed both ways), mverab/eGEOagents (a content-optimisation skills pack) and ivannikov-pro/ai-visibility-tracker (0 stars, self-hosting still marked "planned", no push since 22 May 2026). Two naming traps are documented because both cost real minutes: the npm package `ai-visibility` is a robots.txt/llms.txt generator and not the GitHub project of the same name, and four of the listed projects are not on npm at all.

  **Also honest about the other direction.** A new paragraph names, concretely, what the hosted tools do better — engine breadth above all (Superlines 10+, LLMrefs 11, Trakkr 8, Otterly 7 including Google AI Overviews and AI Mode, against four API-reachable engines here), plus MCP servers, hosted APIs, white-label portals, seats, SSO and SOC-2. "Pick something else when" was rewritten from a single sentence into three yes/no questions that route the reader to a named competitor, on the model of SQLite's *Appropriate Uses* page. A new Schema.org FAQ entry answers "what are the open-source alternatives", because the open-source comparison is the query where this project is currently cited first and the entity block said nothing about it.

  **The counted claims are now pinned to the markup that backs them** (`test/e2e/readme-comparison-truth.test.js`). Both sections make a claim in prose about rows in a table above it, and a first draft shipped the exact contradiction that invites: a ✳ on the Semrush row meaning *its page was read on 4 September*, while the prose forty lines down named Semrush as the one ✳ row that had **not** been capability-checked. The new tests derive the expected numbers from the markup rather than restating them — the set of ✳ rows must equal the set of names the prose enumerates, and the re-check total must equal ✳ rows plus open-source peers plus explicitly-excluded projects — so adding the next competitor cannot silently re-open the hole. Both go RED on an injected regression (star removed, or the total edited), verified before commit. A third pin holds the word *portable* in both column headers and the section heading, since dropping it is what makes the wedge claim false. Twelve Product Hunt candidates logged on 2026-08-19 were not vetted in this pass and are now named in the README as *unknown, not ruled out*, rather than left silently absent.

  **One phrasing note for whoever edits these tables next.** Competitor prices are written "$79 a month", not "$79/mo", because the repo's pre-publish lint treats the `$N/mo` form as a commerce CTA — a guard that exists to keep Webappski's own pricing out of this README, and one worth keeping. No Webappski price appears anywhere here; the rewording preserves the guard's intent while still giving the reader the concrete number that a comparison table is for.

  **Not added, deliberately.** No Webappski row: our own delivery would not score Yes/Yes/Yes in these binary columns honestly — the free hosted audit runs on Webappski's keys in Cloud Functions, so raw data does not stay local, and the plan-equivalent is a separate invoice-gated deliverable rather than something the audit flow hands over. The existing prose about a client being able to install the engine and re-derive their own grid already says the true version, and was left exactly as strong as it was. No Adobe LLM Optimizer row either: its product page timed out twice, and an unreachable page is *unknown*, not *absent*. **[Superseded on 2026-09-07 by 1.12.2 above, as to the Webappski row only — it was added, written honestly, and the reasoning in this paragraph does not survive contact with it: it tested a `Yes/Yes/Yes` row against omission and never tried a truthful one. The Adobe reasoning stands. Text kept verbatim on purpose.]**
