...
Патч. Команды, флаги и коды возврата прежние, числа и их источники тоже. Меняется ТЕКСТ: два блока в `report --white-label`, имя движка в одной оговорке и подпись заголовочной метрики во всех трёх режимах — то есть меняется документ, который видит клиент.
