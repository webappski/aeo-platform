...
Патч. Команды, флаги и коды возврата прежние. Меняется ТЕКСТ двух блоков в `report --white-label` и имя движка в одной оговорке на всех трёх режимах — то есть меняется документ, который видит клиент.
