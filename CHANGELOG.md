...
- **HTML-отчёт показывает разбивку по стадиям воронки**, которую собирал и выбрасывал с тех пор, как секция была написана. Для всех уже сохранённых прогонов это no-op: секция не рендерится, если ни одна ячейка не несёт `tag`, а таких прогонов на диске нет ни одного — ни одного существующего отчёта это не меняет. Зато делает достижимой дремавшую утечку white-label, поэтому она закрыта тем же коммитом: подпись секции читала таблицу ЗА клиента («high ToFu, zero BoFu — means AI knows your category but not why to choose you») и называла наш файл конфигурации по имени — совет и отпечаток инструмента в документе, который идёт под чужой легендой и несёт только статистику. В white-label оба уходят, таблица остаётся в обоих режимах: отнять цифры значит сделать документ сломанным, а не сдержанным. Одна формулировка на обе поверхности, не две копии. Утечка жила в markdown-рендерере white-label и просто ни разу не срабатывала, потому что ни один прогон не мог включить секцию. Чего по-прежнему нет — в `TECH_DEBT` пункт 2: теги не проставляет ничто (`init` пишет простые строки, `--help` и README о них молчат), так что форма `{q, tag}` достижима только правкой конфигурации руками после чтения исходника.
